...
lbaint_t ext2fs_set_blk_dev(block_dev_desc_t *rbdd, int part)
{
...
int ext2fs_devread(lbaint_t sector, int byte_offset, int byte_len, char *buf)
{
	...
	if ((sector < 0) ||
		...
		printf(" ** %s read outside partition sector 0x%llx\n",
		       __func__,
		       sector);
		return 0;
	}
