...
#if 0  //ALPHA_CUSTOMZE. for test USB 3.0 , only for DNS-340L
	gppHalData.ctrlRev = mvCtrlRevGet();
#else
	gppHalData.ctrlRev = mvCtrlRevGet_Ver2();
#endif
...
setenv("bootcmd","nand read.e 0xa00000 0x500000 0x400000;nand read.e 0xf00000 0xa00000 0x300000;bootm 0xa00000 0xf00000");
#else
...
int print_cpuinfo (void)
{
	...
	mvBoardNameGet(name);
	printf("Board: %s\n",  name);
#if 0	//ALPHA_CUSTOMIZE. Casper, for test USB 3.0, only for DNS-340L
	mvCtrlModelRevNameGet(name);
#else
	mvCtrlModelRevNameGet_Ver2(name);
#endif
