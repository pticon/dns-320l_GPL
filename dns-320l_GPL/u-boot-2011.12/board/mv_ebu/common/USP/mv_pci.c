...
void pci_init_board(void)
{
	...
	for (pexIf = 0; pexIf < activePexCount; pexIf++) {
		...
		/* PEX capability */
		//printf("##### Set pexHWInf = %d #####\n", pexHWInf);
		tempPexReg = MV_REG_READ(PEX_CFG_DIRECT_ACCESS(pexHWInf, PEX_LINK_CAPABILITY_REG));
		printf("##### Read pexHWInf = %d , tempPexReg = %d #####\n", pexHWInf, tempPexReg);
		tempPexReg &= ~(0xF);
		if (pexHWInf == 0)
			tempPexReg |= 0x1; //Casper, for force GEN 1.1
		else
			tempPexReg |= 0x2; //alpha.jack, GEN 2.0
		printf("##### Write pexHWInf = %d , tempPexReg = %d #####\n", pexHWInf, tempPexReg);
		MV_REG_WRITE(PEX_CFG_DIRECT_ACCESS(pexHWInf, PEX_LINK_CAPABILITY_REG), tempPexReg);
		
		tempPexReg = MV_REG_READ(PEX_CFG_DIRECT_ACCESS(pexHWInf, PEX_LINK_CAPABILITY_REG));
		printf("##### Read Again pexHWInf = %d , tempPexReg = %d #####\n", pexHWInf, tempPexReg);		
#endif
	...
	if (pexHWInf == 1)  {					
		u_char	val1;
		pci_read_config_byte(0, 0x0, &val1);
		printf("Casper read 0x0 = [%02x]\n", val1);
		
		pci_read_config_byte(0, 0x1, &val1);
		printf("Casper read 0x1 = [%02x]\n", val1);

		pci_read_config_byte(0, 0x2, &val1);
		printf("Casper read 0x2 = [%02x]\n", val1);

		pci_read_config_byte(0, 0x3, &val1);
		printf("Casper read 0x3 = [%02x]\n", val1);

		pci_read_config_byte(0, 0x96, &val1);
		printf("Casper read 0x96 = [%02x]\n", val1);

		pci_write_config_byte(0, 0x96, 0x28);
	    printf("Casper write 0x96 = [0x28]\n");

		pci_read_config_byte(0, 0x96, &val1);
		printf("Casper read 0x96 = [%02x]\n", val1);

		//MV_REG_BIT_SET(0x40090, BIT6);	//change PCI driving
		//MV_REG_BIT_SET(0x40090, BIT4);	//change PCI driving
		//MV_REG_BIT_SET(0x40070, BIT5);	//change PCI driving
		
	}
#endif

#if 0 //ALPHA_CUSTOMIZE
	printf("Set SATA Controller's GPIOs\n");

#define PEX1_MEM_PHYS_BASE    0xb2000000
	
#define V_S_1_Addr        0xa8  // Vendor-Specific 1 Address
#define V_S_1_Data        0xac  //Vendor-Specific 1 Data
	
	//SATA Controller - GPIO Register
#define SATA_GPIO_DOUT        	0x07071020
#define SATA_GPIO_DOUT_ENB      0x07071024
#define SATA_GPIO_DATA_IN_POL   0x0707102C
#define SATA_GPIO_DATA_IN     	0x07071030
#define SATA_GPIO_INT_CAUSE     0x07071034
#define SATA_GPIO_INT_MASK      0x07071038
#define SATA_GPIO_INT_LEVEL_MSK 0x0707103C
#define SATA_CMODE          	0x07071050
#define SATA_GPIOSAR          	0x07071054
#define SATA_PORT_ACTIVE_EN     0x07071058
	
	//SATA Controller - GPIO - Port Active Select
#define ALL_LINK_ACT  	0x0
#define SATA0_LINK_ACT  0x1
#define SATA1_LINK_ACT  0x2
#define SATA2_LINK_ACT  0x3
#define SATA3_LINK_ACT  0x4
#define GPIO_OUT_MODE 	0x5

	//MV_ULONG value = 0x0;

	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Addr)) = (MV_ULONG )SATA_GPIO_DOUT_ENB;
	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Data)) = (MV_ULONG )0xffffff00;

	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Addr)) = (MV_ULONG )SATA_GPIO_DOUT;
	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Data)) = (MV_ULONG )0x00000000;

	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Addr)) = (MV_ULONG )SATA_PORT_ACTIVE_EN;
	*((MV_ULONG  *)(PEX1_MEM_PHYS_BASE+V_S_1_Data)) = (MV_ULONG )0x2db6db6d;
...
#define GPP_DATA_OUT_REG(grp)		(MV_GPP_REGS_BASE(grp) + 0x00)
	...
	MV_REG_BIT_SET(GPP_DATA_OUT_REG(1), BIT_19); // MPP 51
