...
void douser(const char *username)
{
    struct passwd *pw = NULL;
     char destaddr[32];
//alpha.jack.20100629+ for pop a widown to user login    
    char share_root[64];
    char ftp_home[64];
    DIR *dir;
    struct dirent *ptr;
...
# endif
    ...
    logfile(LOG_INFO, "%s Logout", (!strcmp(account, "ftp")) ? "Anonymous" : account);
    doreply();
#ifdef WITH_BONJOUR
    refreshManager();
#endif
...
//                    logfile(LOG_INFO, "%s\n", cli_addr);
					...
					tryings=0;
				if(retries<=tryings) {// Write incoming IP to block list
