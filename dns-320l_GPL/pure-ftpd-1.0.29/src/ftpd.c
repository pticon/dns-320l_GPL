...
// tasha 2014.10.29 for the bug that  upload image to an FTP server is not available .
void die_timeout(const int err, const int priority, const char * const format, ...)
{
    va_list va;
    char line[MAX_SYSLOG_LINE];
...
void douser(const char *username)
{
    struct passwd *pw = NULL;
     char destaddr[48]; // nelson+ IPV6 address max 45 bytes + [] + '/0' = 48
...
//Alpha.Andrew_modify_20130619 for WD to support admin user
	...
	if ( find == Get_Share(username) )
	{
		addreply_noformat(331, MSG_ANY_PASSWORD);
		return;
	}
//alpha.jack.20100629+ end
...
void dopass(char *password)
{
    ...
    char *hd;
    int ngroups;
    char countbuf[8], destaddr[48];// nelson+ IPV6 address max 45 bytes + [] + '/0' = 48
