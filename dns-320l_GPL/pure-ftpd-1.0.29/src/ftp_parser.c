...
# include <dmalloc.h>
#endif
//extern char client_language;  //jack
extern char tls_login;	//	alpha.patrick 20100504 ++
...
void parser(void)
{
    ...
    for (;;) {
        ...
        case -1:
#ifdef BORING_MODE
         die_timeout(421, LOG_INFO, MSG_TIMEOUT); // tasha 2014.10.29 for the bug that  upload image to an FTP server is not available .
            //die(421, LOG_INFO, MSG_TIMEOUT);
#else
          die_timeout(421, LOG_INFO, MSG_TIMEOUT_PARSER);    // tasha 2014.10.29 for the bug that  upload image to an FTP server is not available .
...
//#endif
	...
	arg = narg;
        ...
        if (!strcmp(cmd, "user")) {
            ...
            if (tls_cnx == NULL) {
                (void) tls_init_new_session();
                tls_login='1'; //alpha.patrick 20100504
            }
            goto wayout;
