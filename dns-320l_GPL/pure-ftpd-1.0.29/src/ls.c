...
static int listfile(const PureFileInfo * const fi,  const char *name)
{
    ...
    if (fi == NULL) {
       ...
       {
	       if ( check_hidden_folder(name) == 1 )
		    {
		    	return 0;
		    }
		 }
		 
		 if ( Check_Share_List(name) == -1 )
	 		return -1;
...
# endif
...
#endif
    if (t == NULL) {
        //logfile(LOG_ERR, "{gm,local}gtime() for [%s]", name); /*alpha.jack.20100921-*/
        return 0;
    }
    if (opt_F) {
    ...
    if (opt_l) {
        ...
        case S_IFREG:
            m[0] = '-';
            break;
//jack20101029+ for WSFTP not support share foler is link.
//        case S_IFLNK:
//            m[0] = 'l';
//            break;            /* readlink() here? */
        case S_IFLNK:    
...
//alpha.jack.20100902- for windows explorer connnect ftp server , share folder incorrect            
//            if (S_ISLNK(st.st_mode)) {
