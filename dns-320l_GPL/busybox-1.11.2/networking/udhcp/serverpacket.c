...
int send_offer(uint8_t *hostname, struct dhcpMessage *oldpacket)
{
	...
	if (!static_lease_ip) 
	{
		DBG_PRINT(printf("[serverpacket] !static_lease_ip\n");)
		...
		else 
		{
			...
			DBG_PRINT(printf("[serverpacket] find_address(0)\n");)
...
int send_ACK(uint8_t *hostname, struct dhcpMessage *oldpacket, uint32_t yiaddr)
{
	struct dhcpMessage packet;
	struct option_set *curr;
	uint8_t *lease_time;
