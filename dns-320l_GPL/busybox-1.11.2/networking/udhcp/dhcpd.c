...
static uint8_t hostname[MAX_HOSTNAME_LEN];	//darren20080909 follow wilson
...
int udhcpd_main(int argc ATTRIBUTE_UNUSED, char **argv)
{
	...
	static uint8_t *host = NULL;	//darren20080909 follow wilson
	unsigned int len = 0; //darren20080909 follow wilson
#ifdef DBG
	struct in_addr ip;
#endif
	...
	CLIENT_PORT = 68;
#endif
	DBG_PRINT(printf("[dhcpd] getopt32\n");)
	opt = getopt32(argv, "fS" USE_FEATURE_UDHCP_PORT("P:", &str_P));
	argv += optind;

	DBG_PRINT(printf("[dhcpd] 	bb_daemonize_or_rexec\n");)
#if 0
	if (!(opt & 1)) { /* no -f */
...
#endif
	DBG_PRINT(printf("[dhcpd] openlog\n");)
	if (opt & 2) { /* -S */
...
#endif
	DBG_PRINT(printf("[dhcpd] read_config\n");)
	...
	DBG_PRINT(printf("[dhcpd] write_pidfile\n");)
...
#ifdef DBG
	ip.s_addr = htonl(server_config.server);
	printf("[dhcpd]server_ip=%s\n",inet_ntoa(ip));
#endif
	
	DBG_PRINT(printf("[dhcpd] Setup the signal pipe\n");)
	/* Setup the signal pipe */
	udhcp_sp_setup();

	timeout_end = monotonic_sec() + server_config.auto_time;
	while (1) { /* loop until universe collapses */
		DBG_PRINT(printf("[dhcpd] loop until universe collapses\n");)
		...
		case DHCPDISCOVER:
			...
			DBG_PRINT(printf("[dhcpd] DHCPDISCOVER\n");)
			//darren20080909
			if ((host = get_option(&packet, DHCP_HOST_NAME)) == NULL) {
				DEBUG("couldn't get option from packet, ignoring");
				...
				DEBUG("len = %d, client's hostname = %s\n", len, hostname);
			}
			//darren20090909 end
			DBG_PRINT(printf("[dhcpd] send_offer\n");)
			if (send_offer(&hostname[0],&packet) < 0) {
				...
				if (server_id) {
					...
					if (server_id_align == server_config.server && requested
					 && requested_align == lease->yiaddr
						...
						send_ACK(&hostname[0],&packet, lease->yiaddr);
					}
				} else if (requested) {
					/* INIT-REBOOT State */
