...
save_camera_file_to_file (
	...
	if (curname) {
		...
		chmod(s,0666 & ~x);
		chmod(s, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);		//+++ jeffrey
	}
		   ...
		   CameraFileType type)
{
	...
	fd = mkstemp(tmpname);
//	chmod(tmpname, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);		//+++ jeffrey
	chmod(tmpname, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);		//+++ jeffrey
	if (fd == -1) {
        	CR (gp_file_new (&file));
		tmpfilename = NULL;
	} else {
	...
	//alpha.jeffrey20110629   (for  Kodak Easyshare C140/M1063)
	struct stat sbuf;
	memset(&sbuf, 0, sizeof(struct stat));
	stat(tmpfilename, &sbuf);
//-	if (res < GP_OK) {
	if (res < GP_OK || sbuf.st_size == 0) {
		gp_file_unref (file);
...
//		params->p.r = num_files_action (&gp_params);
		params->p.r = for_each_folder (&gp_params, num_files_action);	//jeffrey
		break;
	case ARG_MKDIR:
