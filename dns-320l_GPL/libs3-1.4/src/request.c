...
#define uri_append(fmt, ...)                                                 \
    ...
    if (bucketContext->bucketName && 
        ...
        char *uri = S3_HOSTNAME;
        if(bucketContext->location && bucketContext->location[0]){
        	char uriadd[64];
        	uri = uriadd;
        	if(!strcmp(bucketContext->location,"EU")){
        		sprintf(uri,"s3-eu-west-1.amazonaws.com");
        	}else{
        		sprintf(uri,"s3-%s.amazonaws.com",bucketContext->location);
        	}	
        }
        if (bucketContext->uriStyle == S3UriStyleVirtualHost) 
        	uri_append("%s.%s", bucketContext->bucketName, uri);
        else
            uri_append("%s/%s",uri, bucketContext->bucketName);
    }
    else {
...
#define curl_easy_setopt_safe(opt, val)                                 \
	...
	// 2012.02.06 - zoe_sun@alphanetworks.com
	// Set timeout 30 seconds
	curl_easy_setopt_safe(CURLOPT_TIMEOUT, 30);
	// 2012.02.06 - zoe_sun@alphanetworks.com
