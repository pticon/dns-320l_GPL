/* $Id: read_tif.c,v 1.1.1.1 2009/10/09 02:34:38 jack Exp $ */
...
int	readdata = 1;			/* read data in file */
...
TIFF *read_tif(char *in_file)
{
	...
	int chopstrips = 0;		/* disable strip chopping */
		...
		//tif = TIFFOpen(argv[optind], chopstrips ? "rC" : "rc");
		tif = TIFFOpen(in_file, chopstrips ? "rC" : "rc");
		if (tif != NULL) 
		{
			if (dirnum != -1) {
				if (TIFFSetDirectory(tif, (tdir_t) dirnum))
...
//
//	setbuf(stderr, buf);
////        fprintf(stderr, "%s\n\n", TIFFGetVersion()); //Vincent03072006
...
TIFFReadContigStripData(TIFF* tif)
{
	...
	int	i=0,j=0,k=0,x=0,value; //Vincent03072006
	...
	if (buf) {
		...
		uint32	width; //Vincent03072006
		uint32	bps=0; //Vincent03072006
		
		TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &width); //Vincent03072006
		
		TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &h);
		TIFFGetField(tif, TIFFTAG_BITSPERSAMPLE, &bps);

//		printf("w %d, h %d,bps %d,TIFFStripSize(tif) %d\n",width, h, bps, TIFFStripSize(tif));
		RGB_buffer=(colorRGB *)(malloc(width*h*4));
		memset(RGB_buffer,0,sizeof(RGB_buffer));
	 	if (!RGB_buffer)
	 	{
			printf("Not enough memory for the tif image.\n");
			_TIFFfree(buf);
			return 0;
 		}
		...
		for (row = 0; row < h; row += rowsperstrip) {
			uint32 nrow = (row+rowsperstrip > h ?
			    h-row : rowsperstrip);
			//printf("\nnrow %d,h-row %d,rowsperstrip %d,scanline %d\n",nrow,h-row,rowsperstrip,scanline);
			...
			{
				if(bitpersample == 8) // 256
				{
//					printf("j %d,strip %d\n",j,strip);
					for (i = 0; i < nrow; i++)
					{
						for (j = i*scanline; j < (i*scanline)+scanline; j++)
						{
							RGB_buffer[k].R = buf[j];
//							printf("%d ",buf[j]);
							RGB_buffer[k].G = buf[j];
//							printf("%d ",buf[j]);
							RGB_buffer[k].B = buf[j];
//							printf("%d ",buf[j]);
							k++;
						}
					}
					//printf("j %d,k %d,strip %d\n",j, k,strip);					
				}
				else if(bitpersample == 1) // 1
				{
//					printf("j %d,strip %d\n",j,strip);
					for (i = 0; i < nrow; i++)
					{
...
//							printf("%d-/",buf[j]);
							for (x = 0; x < 8; x++)
							{
								value = ( ((buf[j] << x) & 0x0080)  >>7) ;
								
								if (value == 1)
									value = 255;
//								printf("%d ",value);
								RGB_buffer[k].R = value;
								RGB_buffer[k].G = value;
								RGB_buffer[k].B = value;
								k++;
							}
//							printf("/\n");
						}
...
//		for(i=0;i<width*h;i++)
//		{
//			printf("%d %d %d ",RGB_buffer[i].R,RGB_buffer[i].G,RGB_buffer[i].B);
//		}
//		printf("\n");
	}
}
...
TIFFReadData(TIFF* tif)
{
	...
	if (TIFFIsTiled(tif)) {
		//printf("11TIFFIsTiled(tif)\n");
		if (config == PLANARCONFIG_CONTIG)
		{
			//printf("11PLANARCONFIG_CONTIG\n");
			TIFFReadContigTileData(tif);
		}
		else
		{
			//printf("22PLANARCONFIG_CONTIG\n");
			TIFFReadSeparateTileData(tif);
		}
	} else {
		//printf("22TIFFIsTiled(tif)\n");
		if (config == PLANARCONFIG_CONTIG)
		{
			//printf("11PLANARCONFIG_CONTIG\n");
...
tiffinfo(TIFF* tif, uint16 order, long flags)
{
	if (!readdata)
		return;
	if (rawdata) {
		//printf("tiffinfo(): 11rawdata\n");
		if (order) {
			//printf("tiffinfo(): 11order\n");
			uint16 o;
		...
		{
			//printf("tiffinfo(): 22order\n");
			TIFFReadRawData(tif, 0);
		}
	} else {
		//printf("tiffinfo(): 22rawdata\n");
		if (order)
		{
			//printf("tiffinfo(): 11order\n");
