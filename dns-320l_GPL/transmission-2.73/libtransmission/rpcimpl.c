 ...
 *
 * $Id: rpcimpl.c 13504 2012-09-19 05:11:19Z jordan $
 */
...
#ifdef ALPHA_CUSTOMIZE
#define strcatf(str,x...) sprintf(&(str[strlen(str)]),x)
    do{
        int idx;
        char strMnt[1024];
        char strVols[1024] = "";
        DIR *pDir = NULL;
        for(idx=0;idx<4;idx++){
            sprintf(strMnt, "/mnt/HD/HD_%c2", 'a'+idx);
            pDir = opendir(strMnt);
            if( pDir ){
            	closedir(pDir);
            	if( strVols[0] )
            	    strcatf(strVols, ";");
            	strcatf(strVols, "%d", idx+1);
            }
        }
        tr_bencDictAddStr ( d, "all-volumes",                     strVols );
    }while(0);
#endif
