...
function PrefsDialog(remote) {
    ...
    // ALPHA_CUSTOMIZE
    initVolumeDropDown = function(e, strVols)
    {
        var i;
        var vols = strVols.split(";");
        if( e.options.length==0 && vols.length>0 ){
            for (i=0; i<vols.length; ++i) {
                e.options[i] = new Option("Volume_"+vols[i], parseInt(vols[i]));
            }
        }
    },
	...
	// ALPHA_CUSTOMIZE
	if( ev.target.id=="download-dir" ){
		var val = o[ev.target.id];
		if( val=="1" )
			val = "a";
		else if( val=="2" )
			val = "b";
			...
			val = "d";
		o[ev.target.id] = "/mnt/HD/HD_" + val + "2/Transmission";
	}
        ...
        for (i=0; key=keys[i]; ++i)
        {
            ...
            else switch (e[0].type)
            {
                ...
                case 'select-one':
                    // ALPHA_CUSTOMIZE
                    if( key == "download-dir" ){
                    	initVolumeDropDown(root.find('#download-dir')[0], o['all-volumes']);
                        if( val == "/mnt/HD/HD_a2/Transmission" )
                            val = 1;
                        else if( val == "/mnt/HD/HD_b2/Transmission" )
                            val = 2;
                        else if( val == "/mnt/HD/HD_c2/Transmission" )
                            val = 3;
                        else if( val == "/mnt/HD/HD_d2/Transmission" )
                            val = 4;
                    }
                    e.val(val);
                    break;
