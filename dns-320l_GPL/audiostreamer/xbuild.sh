...
build_libxslt()
{
	...
	./configure --prefix=$PWD/../xinst --host=arm-linux --disable-dependency-tracking --with-python=no --with-crypto=no --with-debug=no --with-debugger=no --with-libxml-include-prefix="$PWD/../../libxml2-2.7.4/include" --with-libxml-libs-prefix="$PWD/../../libxml2-2.7.4/lib" LDFLAGS="-L$PWD/../../zlib-1.2.3"
	make
	if [ ! $? -eq 0 ]; then
		echo ""
...
build_libshout()
{
	echo "Building libshout-2.2.2"
	cd libshout-2.2.2
	patch -p1 < $PWD/../patches/libshout-2.2.2-automagic.patch
	patch -p1 < $PWD/../patches/libshout-2.2.2-asneeded.patch
	libtoolize
...
{
	echo ""
	echo "Usage: ./xbuild.sh <libogg|libvorbis|libxslt|icecast|libshout|lame|ices|all>"
	echo ""
}
