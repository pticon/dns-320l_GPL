...
//+Wilson08072006, removed interactive parameter input method
#if 01
...
//+Wilson08082006
int
//read_hex(struct systypes *sys)	//+Wilson08092006
read_hex(struct systypes *sys, char *ptr)	//+Wilson08092006
{
        int hex;
...
#else
	//printf("read_hex(SysID == 0x%s)\n", ptr);
	hex = 0;
	do
		hex = hex << 4 | hex_val(*ptr++);
	while(isxdigit(*ptr));
...
delete_partition(int i) {
	//struct pte *pe = &ptes[i];
	//struct partition *p = pe->part_table;		//+Wilson08072006
	//struct partition *q = pe->ext_pointer;	//+Wilson08072006
	struct pte *pe = NULL;						//+Wilson08072006
	struct partition *p = NULL, *q = NULL;		//+Wilson08072006
	int times = 0;								//+Wilson08072006
	int all_part = 0;							//+Wilson08092006
 ...
 */
	...
	if (i < 4) {
		...
		clear_partition(p);
			//return;	//+Wilson08092006
	}
		else if (!q->sys_ind && i > 4) {	//+Wilson08092006
...
//static void	//+Wilson08082006
//change_sysid(void) {	//+Wilson08082006
static void	change_sysid(char *c, char *ptr)	//+Wilson08082006
{
	char *temp;
	int i, sys, origsys;
	struct partition *p;
...
//Wilson08092006+
	...
	//+Wilson08082006
	i = (atoi(c) - 1);
	printf("Changed Partition Number = %d \n", atoi(c) );
	//Wilson08082006+
	...
	if (!sys && !sgi_label && !sun_label && !get_nr_sects(p))
	{
		...
		sys = read_hex(get_sys_types(), ptr);	//+Wilson08092006
		printf("Changed SysID = 0x%x \n", sys);	//+Wilson08082006
...
list_table(int xtra) {
	struct partition *p;
	//char *type;	//+Wilson08092006
	//int i, w;		//+Wilson08092006
	int i;			//+Wilson08092006
...
/* type name */		(type = partition_type(p->sys_ind)) ?
	...
	printf("partitions = %d, sys_ind = 0x%x \n", partitions, p->sys_ind);
	if((p->sys_ind) == 0x82)
	{
		//+Wilson08162006
		//system("touch /tmp/swap");
		//system("touch /web/swap");
		system("touch /tmp/swap_exist");
		//Wilson08162006+
	}
	return;
#endif
...
verify(void) {
	...
	if (total > total_number_of_sectors)
		...
		printf(_("%llu unallocated sectors\n"),
		       total_number_of_sectors - total);
}

static void
add_partition(int n, int sys, int stop_addr) {	//+Wilson08092006
...
#ifdef ADV_FORMAT
        unsigned long long count_stop = 0;
#endif
	...
	fill_bounds(first, last);
	if (n < 21) {   //alpaha.jimmy20090716, support for 20 partitions
		start = sector_offset;
...
#else
			start = cround(i);
			printf("start address = %llu, limit %llu \n", start, cround(limit));					 
#endif
//Wilson08092006+
	...
	if (n > 21) {		//alpha.jimmy20090716, support for 20 partitions	 /* NOT for fifth partition */
...
//+Wilson08092006				
		...
		if (stop_addr == -1) {
				...
				stop = cround(start) + ((stop_addr * 2048) / units_per_sector);
		}
#else
		if (stop_addr == -1)
			stop = cround(limit);
...
//Wilson08092006+
			...
			if (display_in_cyl_units && stop_addr != -1) {
...
static void
//new_partition(void) {
new_partition(int disk_num, char *type, int end)	//+Wilson08092006
{
#ifdef ADV_FORMAT
	int i, free_primary = 0,sys;
#else
    struct systypes *sys;	//+Wilson08092006
...
#else
	sys = read_hex(get_sys_types(), type);
	add_partition((disk_num-1), sys, end);
	return;
#endif
//Wilson08092006+
...
//+Wilson08072006, indicate the status
void touch_file(int status, void *arg)
{
    printf("exit status %d \n", status);
    if(status != 0)
    	system("touch /tmp/format_error");	
    else
    	printf("success \n");	
}	
//Wilson08072006+
...
main(int argc, char **argv) {
	int j, c;
	int optl = 0, opts = 0;
	int i = 0;	//+Wilson08072006

	//alpha.darren 20081126
	fprintf (stderr, "*** Starting fdisk...\n");
	while (argv[i])
	...
	on_exit(touch_file, (void *)NULL);		//+Wilson08072006
...
//+Wilson08072006
	...
	if (argc-optind == 1)
	{
		...
		#ifdef DBG
			printf("argc = %d, optind = %d, disk_device = %s\n", argc, optind, disk_device);
		#endif
	}
...
//Wilson08072006+
	...
	//+Wilson08072006
		...
		{
			case 'd': 
				delete_partition(atoi(argv[2+i+1]));
				i += 2;
				break;
			case 'n': 
				new_partition(atoi((argv[2+i+1])), (argv[2+i+2]), atoi((argv[2+i+3])) );
				i += 4;
				break;
			case 'p': 
				list_table(atoi((argv[2+i+1])));
				exit(0);
			case 't': 
				change_sysid((argv[2+i+1]), (argv[2+i+2]));
				i += 3;
				break;
			case 'w': 
				write_table();
