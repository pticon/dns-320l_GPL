...
int get_shadow_passwd(buffer *name, char *pwd)
{
	FILE            *fp;
	struct passwd   *pw;
	int             ret = -1 ;
	...
	while( (pw = fgetpwent(fp)) != 0 ) {
		if( strcmp(pw->pw_name, name->ptr)==0 ) {
			strcpy(pwd,pw->pw_passwd);
			ret = 0;
		}
	}
	fclose(fp);
...
static int http_auth_get_password(server *srv, mod_auth_plugin_data *p, buffer *username, buffer *realm, buffer *password) {
	...
	*/
		...
		ret = get_shadow_passwd(username, tmp);
		//fprintf(stderr , "get %s shadow password .\n",username->ptr);	
		buffer_copy_string_len(password,tmp,strlen(tmp));
...
static int http_auth_match_rules(server *srv, mod_auth_plugin_data *p, const char *url, const char *username, const char *group, const char *host, connection *con) {
...
static int http_auth_basic_password_compare(server *srv, mod_auth_plugin_data *p, array *req, buffer *username, buffer *realm, buffer *password, const char *pw) {
	...
	if (p->conf.auth_backend == AUTH_BACKEND_HTDIGEST) {
		...
		if (!strlen(password->ptr))
			return -1;
		cp = pw_encrypt(pw, password->ptr);
		
			
		if (!strcmp(cp, password->ptr)) {
				buffer_copy_string_buffer(srv->srvconf.username, username);
				return 0;
		}
	} else if (p->conf.auth_backend == AUTH_BACKEND_LDAP) {
#ifdef USE_LDAP
