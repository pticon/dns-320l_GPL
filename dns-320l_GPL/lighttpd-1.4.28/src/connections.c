...
#define UPLOAD_TMP "/var/tmp"
#define FW_TMPFD   "/usr/local/tmp"
#define FW_TMPFS   "/usr/local/upload"
#endif
...
#ifdef ALPHA_CUSTOMIZE
/* Translate to a K scale value . 
 * But we may not need to transfer to Kb .....*/
//static unsigned long kscale(unsigned long b , unsigned long bs)
static unsigned long long bscale(unsigned long long b , unsigned long long bs)
{
	//return (b *(unsigned long long)bs + 1024/2) / 1024 ;
	return (b *(unsigned long long)bs + 1024/2) ;
}

int remain_space(const char *path , unsigned long long content_length)
{
	int ret = 0 ;
	struct statfs s ;
	unsigned long long fremain_space ;	
	...
	//fremain_space = kscale(s.f_bavail , s.f_bsize);
	fremain_space = bscale(s.f_bavail , s.f_bsize);
	
#if 0	
	printf("remain_space = %llu , content_length = %lu.\n",fremain_space , content_length);
#endif
		
	if (fremain_space <= content_length)
		return -1 ;
	
	return 0 ;
}

int nas_upload_temp_lookup(unsigned long long content_length)
{
	char true_path[1024];
	char *true_path_ret = NULL ;
	int  special_flag = 0 ;
	int  i ,j,ret = 0;
	char folder_p[256];
	...
	//true_path == (char *)&true_path | NULL
	true_path_ret = realpath(UPLOAD_TMP , true_path);
	
	/* 1. If no link .
	 * 2. If not a link . */
	if ((!true_path_ret) ||
		(!strncmp(UPLOAD_TMP,true_path,strlen(true_path)))) {
		special_flag = 1 ;
	}
	
	/* if real link path point to /usr/local/tmp , this 
	 * mean the nas may need to do some f/w upgrade action ,
	 * we don't change this part . */
	
	if (!strncmp(true_path , FW_TMPFS , strlen(true_path)) ||
		!strncmp(true_path , FW_TMPFD , strlen(true_path))) {
		//con.space_check_config = 1 ;
		return 0 ;
	}
	...
	if (special_flag ) {
		remove(UPLOAD_TMP);
		for (i = 0 ; i < 4 ; i ++) {
			memset(folder_p , 0 , sizeof(folder_p));
			
			sprintf(folder_p , "/mnt/HD/HD_%c2",'a'+i);
			
			if (!access(folder_p,F_OK)) {
				strcat(folder_p , "/.systemfile/upload");
				if (access(folder_p,F_OK))
					mkdir(folder_p,0777);

				symlink(folder_p , UPLOAD_TMP);
				break ;
			}
		}
	}
	...
	for (i = 0 ; i < 4 ; i ++) {
		...
		if (!strncmp(true_path,folder_p,strlen(folder_p))) {
			ret = remain_space(folder_p ,content_length);
			if (ret < 0) {
				for (j = 0 ; j < 4 ; j ++) {
					sprintf(folder_p , "/mnt/HD/HD_%c2",'a'+j);
					if (remain_space(folder_p,content_length) < 0) {
						continue ;
					}
					else {
						strcat(folder_p , "/.systemfile/upload");
		...
		for (c = cq->first; c && (dst_cq->bytes_in != (off_t)con->request.content_length); c = c->next) {
			...
			if (con->request.content_length > 64 * 1024) {
				...
				if (dst_cq->bytes_in + toRead == (off_t)con->request.content_length) {
					...
					#ifdef ALPHA_CUSTOMIZE
					if (con->temp_space_check_flag)
						con->temp_space_check_flag = 0;
					#endif
				}
			} else {
				buffer *b;
...
#ifdef ALPHA_CUSTOMIZE			
		//Add condition for IE length ,
		//if content length > 512K
		if ((con->state == CON_STATE_READ_POST) &&
			(con->request.content_length > 524288)) {
			if (!con->temp_space_check_flag) {
				int  nas_ret = 0 ;

				nas_ret = nas_upload_temp_lookup((unsigned long long)con->request.content_length);
