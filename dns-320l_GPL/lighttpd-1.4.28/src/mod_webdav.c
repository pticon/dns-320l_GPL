...
#define WEBDAV_FILE_MODE S_IRWXU | S_IRWXG | S_IRWXO
		...
		case 1:
			...
			if (NULL != (dir = opendir(con->physical.path->ptr))) {
#ifdef PROJECT_FEATURE_PARAGON_NTFS_WEBDAV_PATCH
				int dot_showed = 0; // nelson
#endif
				struct dirent *de;
				physical d;
				...
				if (!S_ISLNK(st.st_mode)) {
					strncpy(tmp1 , con->physical.path->ptr , strlen(con->physical.path->ptr));
					if (tmp1[strlen(tmp1)-1] == '/')
						strncpy(tmp2, tmp1 , strlen(tmp1)-1);
					else
						strncpy(tmp2 , tmp1 , strlen(tmp1));

						readlink(tmp2 , bufs , sizeof(bufs) );
						if ((!strcmp(bufs , "/mnt/HD/HD_a2/")) ||
							(!strcmp(bufs , "/mnt/HD/HD_b2/")) ||
							(!strcmp(bufs , "/mnt/HD/HD_c2/")) ||
							(!strcmp(bufs , "/mnt/HD/HD_d2/"))) {
							ignore_tag = 1;
						}
						else {
							ignore_tag = 0;
				...
				while(NULL != (de = readdir(dir))) {
					...
					if (ignore_tag) {
						if (!strcmp(de->d_name , ".!@$mmc")) {
							continue;
						}
						if (!strcmp(de->d_name , ".systemfile")) {
							continue;
						}
						if (!strcmp(de->d_name , "Nas_Prog")) {
							continue;
						}
						if (!strcmp(de->d_name , ".AppleDouble")) {
							continue;
						}
						if (!strcmp(de->d_name , ".AppleDB")) {
							continue;
						}
						if (!strcmp(de->d_name , ".DS_Store")) {
							continue;
						}
						if (!strcmp(de->d_name , ".bin")) {
							continue;
						}
						if (!strcmp(de->d_name , ".AppleDesktop")) {
							continue;
						}
						if (!strcmp(de->d_name , "Network Trash Folder")) {
							continue;
						}
						if (!strcmp(de->d_name , "lost+found")) {
							continue;
						}
						if (!strcmp(de->d_name , ".!@#$recycle")) {
							continue;
						}
						if (!strcmp(de->d_name , ".]")) {
							continue;
						}
						if (!strcmp(de->d_name , "aMule")) {
							continue;
						}
						if (!strcmp(de->d_name , "P2P")) {
							continue;
						}
						if (!strcmp(de->d_name , ".!@#$mydlinkSync")) {
							continue;
						}
					...
					if (de->d_name[0] == '.' && de->d_name[1] == '\0') {
						/* don't append the . */
#ifdef PROJECT_FEATURE_PARAGON_NTFS_WEBDAV_PATCH
						dot_showed = 1; // already showed current folder
#endif
					} else {
						buffer_append_string(d.path, de->d_name);
						buffer_append_string(d.rel_path, de->d_name);
...
#ifdef PROJECT_FEATURE_PARAGON_NTFS_WEBDAV_PATCH
				/* workaround paragon ntfs driver's behavior that there is not any folder
				 * . and ..  
				 */
				if(!dot_showed) {
					dot_showed = 1;
