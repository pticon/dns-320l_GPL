...
URIHANDLER_FUNC(mod_secdownload_uri_handler) {
	...
	rel_uri = ts_str + 8;
#ifdef ALPHA_CUSTOMIZE
	//for security issue:document root could be traversed
	path = buffer_init_string(p->conf.doc_root->ptr);
	buffer_append_string(path, rel_uri + 1);
	if( (real_path = realpath(path->ptr, NULL)) == NULL)
	{
		buffer_free(path);
		return HANDLER_ERROR;
	}
	else if(strncmp(real_path, p->conf.doc_root->ptr, strlen(p->conf.doc_root->ptr)) != 0)
	{
		free(real_path);
		buffer_free(path);
		return HANDLER_ERROR;
