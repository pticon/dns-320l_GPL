...
int disk_sleep =0; /* alpha.jack.20130102+ for sata hd spinup */
...
static ssize_t
show_hd_sleep(struct device *dev,
		  struct device_attribute *attr, char *buf)
{
...
static ssize_t
store_hd_sleep(struct device *dev,
		  struct device_attribute *attr, char *buf , size_t count)
{

	disk_sleep = simple_strtoul(buf, NULL, 0);
	
	return count;
}

static DEVICE_ATTR(hd_sleep, S_IRUGO | S_IWUSR , show_hd_sleep, store_hd_sleep);
...
struct scsi_transport_template *ata_attach_transport(void)
{
	...
	SETUP_TEMPLATE(dev_attrs, hd_sleep, S_IRUGO | S_IWUSR , 1); /*alpha.jack.20130102+ for sata hd spinup */
