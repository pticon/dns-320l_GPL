...
/* alpha.jack/20130102+ for sata hd spinup */
extern int disk_sleep;
int pre_enable_bits=0;
int standby_step = 0;
/* alpha.jack/20130102 end */
...
{
//alpha.jack : marvell patch start for sata host map
...
void check_hd_wakeup(unsigned int port , unsigned int protocol)
{
	unsigned int shift,hardport;
	u32 enable_bits;
	
	MV_PORT_TO_SHIFT_AND_HARDPORT(port, shift, hardport);
	enable_bits  = (DONE_IRQ|ERR_IRQ) << shift;
	
//	printk("enable_bits..=%x\n",enable_bits);
	
	if ( disk_sleep == 0x11 ) 
	{
		if ( protocol == ATA_PROT_NCQ )
		{
			switch(standby_step)
			{
				case 0:
					pre_enable_bits = enable_bits;
					standby_step=1;
					break;
				
				case 1:
					if ( pre_enable_bits != enable_bits )
					{
//						printk("mdelay 10 , data command wakeup hd\n");
						mdelay(10000);	
						disk_sleep = 0;
						standby_step = 0;
						pre_enable_bits = enable_bits;
	...
	else if ( disk_sleep == 0x22 ) 
	{
		switch(standby_step)
		{
			...
			case 1:
				if ( pre_enable_bits != enable_bits )
				{
//					printk("mdelay 10 , any command wakeup hd\n");
					mdelay(10000);	
					disk_sleep = 0;
					standby_step = 0;
					pre_enable_bits = enable_bits;
	...
	else if ( disk_sleep == 0x12 )
	{
		switch(standby_step)
		{
			case 0:
				if ( ( enable_bits == 0x03 ) && ( protocol == ATA_PROT_NCQ ) )
				{
					pre_enable_bits = enable_bits;
					standby_step=1;
				}
				else if ( enable_bits == 0x0c )
				{
					pre_enable_bits = enable_bits;
					standby_step=1;
				}
			...
			case 1:
				if ( pre_enable_bits != enable_bits )
				{
//					printk("mdelay 10 , data cmd wakeup hd1 , any cmd wakeup hd2\n");
					mdelay(10000);	
					disk_sleep = 0;
					standby_step = 0;
					pre_enable_bits = enable_bits;
	...
	else if ( disk_sleep == 0x21 )
	{
		switch(standby_step)
		{
			case 0:
				if ( ( enable_bits == 0x0c ) && ( protocol == ATA_PROT_NCQ ) )
				{
					pre_enable_bits = enable_bits;
					standby_step=1;
				}
				else if ( enable_bits == 0x03 )
				{
					pre_enable_bits = enable_bits;
					standby_step=1;
				}
...
static unsigned int mv_qc_issue_fis(struct ata_queued_cmd *qc)
{
	...
	u32 fis[5];
	int err = 0;
	unsigned int port = ap->port_no; /* alpha.jack.20130102+ for sata hd spinup */
	...
	if ( disk_sleep != 0 )
	{
		check_hd_wakeup(port,qc->tf.protocol);
	}
	/* alpha.jack.20130102 end */
...
static int mv_host_intr(struct ata_host *host, u32 main_irq_cause)
{
	...
	//	u32 hc_cause = (main_irq_cause >> shift) & HC0_IRQ_PEND;
      	if (port == 0) {	/* first port on this hc ? */
			u32 hc_cause = (main_irq_cause) & HC0_IRQ_PEND;
			//alpha.jack : marvell patch end
