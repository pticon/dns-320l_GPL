...
extern int mv_eth_ioctl(struct net_device *dev, struct ifreq *rq, int cmd); //alpha.jack.20110504+
...
#endif
	.ndo_do_ioctl		= mv_eth_ioctl, //alpha.jack.20110504+
};
...
static int mv_eth_probe(struct platform_device *pdev)
{
...
#if 0 //Casper, for test
		mv_eth_ports_num = 1 ;;//mvCtrlEthMaxPortGet();
#else
	...
	/* alpha.jack.20110504+ control phy led */
	/*mvEthPhyRegWrite( 0,  22 , 3 );
	mvEthPhyRegWrite( 0,  16 , 0x1017 );
	mvEthPhyRegWrite( 0,  17 , 0x4410 );
	mvEthPhyRegWrite( 0,  22 , 0 );*/
	/* alpha.jack.20110504 end */
...
#endif /* CONFIG_MV_ETH_SWITCH */
		dev->netdev_ops = &mv_eth_netdev_ops;
		strcpy(dev->name ,"egiga%d");  //alpha.jack.20110509 modified
