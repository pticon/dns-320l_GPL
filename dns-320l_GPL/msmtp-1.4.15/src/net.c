	...
	char **canonical_name, char **address, char **errstr)
{
   ...
   //modify by alpha ibuki
//    hints.ai_family = PF_UNSPEC;
    hints.ai_family = PF_INET;
//modify by alpha ibuki end
	...
	*errstr = xasprintf(_("cannot locate host %s: %s"),
	...
	//printf("cannot locate host %s: %s", hostname, wsa_strerror(WSAGetLastError() ); //jerry
#else
	...
	else
	{
	    *errstr = xasprintf(_("cannot locate host %s: %s"), hostname,
		...
		printf("cannot locate host %s: %s\n", hostname, error_code == EAI_SYSTEM ? strerror(errno): gai_strerror(error_code) ); //jerry
	}
#endif
