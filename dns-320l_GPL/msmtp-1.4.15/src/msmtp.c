...
#include <errno.h>
#include <time.h>
//add alpha.ibuki 2011 06 01 start
#include <signal.h>
//#include <sys/time.h>
//add alpha.ibuki 2011 06 01 end
...
void msmtp_print_help(void)
{
    printf(_("USAGE:\n\n"
	    ...
	    "\n\n"
	    "Other Options:\n"
	    "  --pid \"pid file path\"      Save pid to file.\n"
	    "  --force_timeout \"seconds\"  Force EXIT after the assigned seconds.\n\n"
	    //add alpha.ibuki 2011 06 01 end
	    "\nReport bugs to <%s>.\n"),
...
#define LONGONLYOPT_PID		200  //add alpha.ibuki 2011 06 01
#define LONGONLYOPT_FORCE_TIMEOUT		201  //add alpha.ibuki 2011 06 01
...
int msmtp_cmdline(msmtp_cmdline_conf_t *conf, int argc, char *argv[])
{
	...
	{ "pid",									 required_argument, 0, LONGONLYOPT_PID }, //add alpha.ibuki 2011 06 01
	{ "force_timeout",         required_argument, 0, LONGONLYOPT_FORCE_TIMEOUT }, //add alpha.ibuki 2011 06 01
	{ 0, 0, 0, 0 }
    };
    int rcptc;
    char **rcptv;
...
//add alpha.ibuki 2011 06 01 start
			case LONGONLYOPT_PID:

		memset(kill_pid_file,0,sizeof(kill_pid_file));

		sprintf(kill_pid_file,"echo \"kill -9 %d\" > %s",getpid(),optarg);
		system(kill_pid_file);

		break;

			case LONGONLYOPT_FORCE_TIMEOUT:

    signal(SIGALRM, force_timeout);

    itv.it_interval.tv_sec = atoi(optarg);
			...
			&errmsg, &errstr)) != EX_OK)
	{
		...
		if (account->id && account->conffile)
		{
		    print_error(_("could not send mail (account %s from %s)"),
		    	    account->id, account->conffile);
			printf("could not send mail (account %s from %s)\n", account->id, account->conffile ); //jerry
		}
		else
		{
