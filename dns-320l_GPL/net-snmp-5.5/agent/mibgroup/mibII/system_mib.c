...
static size_t   sysObjectID_sizeof;	//20100630 roy add
...
system_parse_config_sysObjectID(const char *token, char *cptr)
{
    ...
    sysObjectID_sizeof = sysObjectIDLength * sizeof(oid);		//20100630 roy add
}
...
init_system_mib(void)
{
    ...
    }
    {
		...
		MAX_OID_LEN, &sysObjectID_sizeof));	//20100630 roy add
		//MAX_OID_LEN, &sysObjectIDLength));	//roy mask
    }
    {
