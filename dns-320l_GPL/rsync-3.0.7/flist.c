...
extern int64 dry_total_size; // zoe 20131126 for write_progress_file 
...
static void count_file_list(char *path)
{
	char cmd[1024];
	FILE *fp;
	...
	fp = popen(cmd,"r");
	fgets(cmd,1024,fp);
	dry_total_size = my_strtoull(cmd);
	pclose(fp);

	//if (verbose > 2)		
		printf("dry_total_size:%llu\n",dry_total_size);
...
struct file_list *send_file_list(int f, int argc, char *argv[])
{
	...
	if(!dump_size){
	snprintf(stats.orig_dir,256,*argv);  // zoe 20131126 for write_progress_file
	count_file_list(stats.orig_dir);
	}
