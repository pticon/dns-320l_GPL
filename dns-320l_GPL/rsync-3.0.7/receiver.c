...
int recv_files(int f_in, char *local_name)
{
...
#endif
	int ndx, recv_ok;
	char fail_flag_file[64];
	char logfile[128]; //tamas add, for log

	if(tmp_file_name != NULL){
		sprintf(logfile,"%s.log", tmp_file_name);//tracy 20151229 modify
		if(access(logfile, 0) == 0){
			log_file_fp = fopen(logfile, "w");
		}
	}
	...
	while (1) {
		...
		//tamas add for log witch file backup failed
		if(tmp_file_name != NULL)
		{
			sprintf(fail_flag_file,"%s_flag", tmp_file_name);
			if(access(fail_flag_file,F_OK) == 0)
			{
				fail_flag_fp = fopen(fail_flag_file, "w+");
				fwrite(fname, strlen(fname)+1, 1, fail_flag_fp);
				fclose(fail_flag_fp);
				fail_flag_fp=NULL;
			}
		}
		...
		case 0: {
			...
			if (msgtype == FERROR_XFER || verbose) {
				...
				if(log_file_fp != NULL){
					char log[1024];
					snprintf(log, sizeof(log), "Error: Failed to backup [%s]\n", fname);
					fputs(log, log_file_fp);//tracy 20151229 modify
				}
			}
			if (!redoing) {
				if (read_batch)
	...
	if(log_file_fp != NULL)
		fclose(log_file_fp);//tracy 20151229+ for local backup logs.
