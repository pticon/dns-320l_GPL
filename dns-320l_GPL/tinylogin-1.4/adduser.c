...
/*alpha.jack.20100712+*/
	...
	user = getpwnam(p->pw_name);
	if ( user != NULL )
		return 1;
		
	/*alpha.jack.20100712+ save uid for quota*/
	if (( fp = fopen("/etc/uid","r") ) != NULL )
	{
		fscanf(fp,"%d",&last_uid);	
		fclose(fp);
		#ifdef DBG
		printf("last uid = %d\n",last_uid);
		#endif
...
//	printf("p->pw_uid=%d\n",p->pw_uid);
	...
	while(1)
	{
		user = getpwuid(last_uid);
		if ( user == NULL )
			break;
		if ((last_uid >= max) || (last_uid < min))
			last_uid= min;
		else
			last_uid++;
	}
...
static int my_adduser(const char *filename, struct passwd *p, char *pwd)
{
	...
	/* alpha.jack.20091013 modify ftp multi group */	
		/* Set the owner and group so it is owned by the new user. */
//		if (chown(p->pw_dir, p->pw_uid, p->pw_gid)) {
//			perror_msg("%s", p->pw_dir);
//		}
	        if (chown(p->pw_dir, 0, 0)) {
			perror_msg("%s", p->pw_dir);
		}
		/* Now fix up the permissions to 2755. Can't do it before now
		 * since chown will clear the setgid bit */
...
//jack20060505 end			
	...
	if (!home) {
		memset(default_home_prefix, 0, sizeof(default_home_prefix));
		#ifdef TWO_BAY_NAS
		if ( access("/tmp/hddok_a",F_OK) == 0 )
		{
			strcpy(default_home_prefix,"/mnt/HD_a4/ftp");
			hddok = 1;
		}
		else if ( access("/tmp/hddok_b",F_OK) == 0 )
		{
			strcpy(default_home_prefix,"/mnt/HD_b4/ftp");
			hddok = 2;
		}
		#else
		
//		fp=fopen("/etc/shared_name", "r");
//		if(fp && !feof(fp) && ! ferror(fp)) {
//			while(fscanf(fp, "%*[^=]%*[^_]_%c%*[^\n]%*c", &hdd_index)!=EOF) {
//				if(flag>hdd_index) 
//					flag=hdd_index;
//			}
//			fclose(fp);
//		}
//	
//		sprintf(default_home_prefix, "/mnt/HD_%c4/ftp", flag);
		strcpy(default_home_prefix, "/usr/share/ftp");
		hddok=1;
		#endif	
...
/* $Id: adduser.c,v 1.6 2010/08/23 11:24:28 jack Exp $ */
