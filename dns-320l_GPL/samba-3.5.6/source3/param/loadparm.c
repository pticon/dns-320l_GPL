...
bool utf8_compare(const char *s1, const char *s2)
{
	...
	//these two length must be the same
	if(s1_len != s2_len)
		return false;
	...
	for(i=0; i<s1_len; i++)
	{
		if(s1[i] < 0x80)
		{
			...
			if(s1[i] > s2[i] && (s1[i] == s2[i]+0x20))
				continue;
			else if(s1[i] < s2[i] && (s1[i] == s2[i]-0x20))
				continue;
			else
				return false;
		}
		else
		{
			...
			//2 bytes or 3 bytes in utf8 char
			if(byte == 2)
			{
			...
			{
				//20091224 roy
				if(s1[i+1] != s2[i+1] || s1[i+2] != s2[i+2])
					return false;
				//20091224 roy end
