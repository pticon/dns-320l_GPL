                                              ...
                                              struct auth_serversupplied_info **presult)
{
		...
		struct auth_serversupplied_info *result;
				...
				//royc 20100503 ==========================================================
				syslog(LOG_INFO, CIFS_AUTH_FAIL, "nobody");
				//royc end ==========================================================
				...
				//royc 20100503 ==========================================================
				syslog(LOG_INFO, CIFS_AUTH_FAIL, vuid_serverinfo->unix_name);
				//royc end ==========================================================
...
/* USER Activites: */
	...
	if(conn->server_info->unix_name != NULL && conn != NULL &&
		!(strncmp(lp_servicename(snum), "IPC$", strlen("IPC$")) == 0
		 || strncmp(lp_servicename(snum), "lp", strlen("lp")) == 0 ) )
	{
		DEBUG(0, (CIFS_CONNECT, 
		  conn->client_address, lp_servicename(snum), conn->server_info->unix_name));
		  
		syslog(LOG_INFO, CIFS_CONNECT, conn->client_address, lp_servicename(snum), conn->server_info->unix_name);
	}
	//royc end ==========================================================
...
void close_cnum(connection_struct *conn, uint16 vuid)
{
	...
	if(conn != NULL && 
		!(strncmp(lp_servicename(SNUM(conn)), "IPC$", strlen("IPC$")) == 0
		 || strncmp(lp_servicename(SNUM(conn)), "lp", strlen("lp")) == 0 ) )
	{
		DEBUG(0, (CIFS_DISCONN, 
		  conn->client_address, lp_servicename(SNUM(conn))));
		  
		syslog(LOG_INFO, CIFS_DISCONN, conn->client_address, lp_servicename(SNUM(conn)));
	}
	//royc end ==========================================================
