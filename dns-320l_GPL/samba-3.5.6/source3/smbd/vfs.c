...
/****************************************************************************
 ...
 Returns 0 on success, -1 on failure.
****************************************************************************/
int vfs_set_filelen_2(files_struct *fsp, SMB_OFF_T len)
{
	int ret;
	...
	// alpha.zoe.20110609+  
	// when you stop the SMB_VFS_FTRUNCATE, the file size shows from 0 on Windows
	// We can set the len to 0 directly, because some windows for example VISTA will allocate 0 size for the file only.  
	// When file size > 100 MB, even VISTA is ok.
	SMB_OFF_T mylen;
	mylen = len;
	if( len > 104857600 ){ // 100 MB
	...
	if ((ret = SMB_VFS_FTRUNCATE(fsp, mylen)) != -1) {	
		set_filelen_write_cache(fsp, len);
