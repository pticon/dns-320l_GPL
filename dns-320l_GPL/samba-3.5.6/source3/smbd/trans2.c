				  ...
				  bool fail_after_createfile)
{
	...
	DEBUG(6,("smb_set_file_size: size: %.0f \n", (double)size));

	if (size == get_file_size_stat(psbuf)) {
		return NT_STATUS_OK;
	}

	//DEBUG(1, ("roy debug %s %s\n", smb_fname_str_dbg(fsp->fsp_name), conn->connectpath));
...
//			return map_nt_error_from_unix(errno);
//		}
		if(strstr(conn->connectpath, "/mnt/USB") != 0)
		{
			//USB device
			if (vfs_set_filelen_2(fsp, size) == -1) {
				return map_nt_error_from_unix(errno);
			}
		}
		else
