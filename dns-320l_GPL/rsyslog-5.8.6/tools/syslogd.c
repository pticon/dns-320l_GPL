...
#if defined(ALPHA_CUSTOMIZE)
		pMsg = (msg_t*) pBatch->pElem[i].pUsrp;
		retVal = xmldbc_get_wb(NULL, 0, "/syslog_server/email/enable", strVal, 512);
		if ( (strlen(strVal) > 0) && (atoi(strVal) == 1) ) {
			retVal = xmldbc_get_wb(NULL, 0, "/syslog_server/email/severity", strVal, 512);
			if (atoi(strVal) == 5) 
				email_severity = 4;
			else 
				email_severity = atoi(strVal);

			if ( ((pMsg->iSeverity) <= email_severity ) && (pMsg->iLenHOSTNAME > 0) ){
				msgLen = pMsg->iLenRawMsg; //strlen(pMsg->pszRawMsg);
				gettimeofday(&tv, NULL);
				ptm = localtime (&tv.tv_sec);
				strftime (time_str, sizeof (time_str), "%Y%m%d_%H%M%S", ptm);
				millisec =tv.tv_usec / 1000;
				srand(time(NULL));
				rand_num=rand();

				sprintf(strVal2, "cp -af /usr/local/modules/usrsbin/syslog_msg_file.txt /tmp/syslog_msg_file_%s_%03ld_%d.txt", time_str, millisec, rand_num);
				system(strVal2);
				sprintf(strVal2, "/tmp/syslog_msg_file_%s_%03ld_%d.txt", time_str, millisec, rand_num);
				sprintf(strVal3, "/tmp/Syslog_Email_Notification_%s_%03ld_%d.txt", time_str, millisec, rand_num);
				fp = fopen(strVal3, "w");
				for (i = 0 ; i < msgLen ; i++) {
					fputc(pMsg->pszRawMsg[i],fp);
				}
				fclose(fp);
								
				sprintf(strVal, "syslog_email.sh %s %s &",strVal2 , strVal3);
				system(strVal);
			}
		}
		DBGPRINTF("msgConsumer Casper :  pMsg->pszRawMsg = [%.60s], pMsg->iSeverity = [%d], [%d], [%s]\n", pMsg->pszRawMsg, pMsg->iSeverity, pMsg->iLenRawMsg, pMsg->pszHOSTNAME);
#endif		
	}
	RETiRet;
}
