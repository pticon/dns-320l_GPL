...
#if defined(PROJECT_FEATURE_LOGS_SERVER_SQLITE3DB)
	uchar *sqlite3DbDir;		/* sqlite3 database dir to use */
#endif
} instanceData;
...
#if defined(PROJECT_FEATURE_LOGS_SERVER_SQLITE3DB)
static uchar *sqlite3DbDir = NULL;	/* sqlite3 database dir to use */
#endif
...
#		endif
	...
	//errmsg.LogError(0, RS_RET_SUSPENDED, "libdbi error: dbiDrvrDir = %s\n",dbiDrvrDir);
...
#if defined(PROJECT_FEATURE_LOGS_SERVER_SQLITE3DB)
		if(pData->sqlite3DbDir != NULL)
			dbi_conn_set_option(pData->conn, "sqlite3_dbdir", (char*) pData->sqlite3DbDir);
#endif
...
#if defined(PROJECT_FEATURE_LOGS_SERVER_SQLITE3DB)
	if(sqlite3DbDir!= NULL)
			if((pData->sqlite3DbDir		= (uchar*) strdup((char*)sqlite3DbDir))		 == NULL) ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
#endif
...
#if defined(PROJECT_FEATURE_LOGS_SERVER_SQLITE3DB)
	CHKiRet(omsdRegCFSLineHdlr(	(uchar *)"actionlibdbisqlite3_dbdir", 0, eCmdHdlrGetWord, NULL, &sqlite3DbDir, STD_LOADABLE_MODULE_ID));	
#endif
