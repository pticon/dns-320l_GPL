...
//alpha.jack.20110509+
static int mv_rtc_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	//struct rtc_time tm;
	MV_RTC_TIME rtc_time;
	void __user *uarg = (void __user *) arg;
	
	switch (cmd) {
		case RTC_RD_TIME:
			memset(&rtc_time,0,sizeof(rtc_time));
			mvRtcTimeGet(&rtc_time);
			err = copy_to_user(uarg, &rtc_time, sizeof(MV_RTC_TIME));
			return err;
		
		case RTC_SET_TIME:
			if (copy_from_user(&rtc_time, uarg, sizeof(MV_RTC_TIME)))
				return -EFAULT;
			mvRtcTimeSet(&rtc_time); //CPU register
			return err;
	}
	return err;
}
//alpha.jack.20110509 end

static struct rtc_class_ops rtc_ops = {
        .read_time      = mv_rtc_read_time,
        .set_time       = mv_rtc_set_time,
        .ioctl          = mv_rtc_ioctl, //alpha.jack.20110505+
};
