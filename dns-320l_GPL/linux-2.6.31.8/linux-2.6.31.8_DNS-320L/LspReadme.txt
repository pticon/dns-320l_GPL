...
To create jffs2 image for NAND flash(with eraseblock size 0x20000) for 1 bit ECC support: 
...
To unprotect all sectors: "./flash_unlock /dev/mtd1"
  ...
  Note: 
  -----
  In case jffs2 partition is required on NAND flash along with 4 bit ECC support, the best way
  to create such partiton is thru NFS/HDD by the following procedure:
	# mount -t jffs2 /dev/mtdblock<i> /mnt 
	# cp -a <ARM_FS>/* /mnt
	# umount /mnt

  When using mkfs.jffs2 to create such partition, all padded pages in the image that might be 
  wriiten then read by the jffs2 layer will result in 4 bit uncorrectble error, since ECC was
  calculated and stored for these padded pages(with 0xFF) as well.
  During write operation, new ECC will be calculated and written to the non-erased OOB section,
  this results in corrpted ECC which be failed in next read operation. 
