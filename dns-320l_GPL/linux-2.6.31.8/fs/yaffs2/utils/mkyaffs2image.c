...
const char * mkyaffsimage_c_version = "$Id: mkyaffs2image.c,v 1.1 2010-06-04 22:38:22 alin Exp $";
...
static int write_chunk(__u8 *data, __u32 objId, __u32 chunkId, __u32 nBytes)
{
	...
	unsigned char spare_data[spareSize];
	...
	memset(spare_data, 0xff, spareSize);
...
// added NCB **CHECK**
	...
	memcpy(spare_data+8, &pt, sizeof(yaffs_PackedTags2));
//	return write(outFile,&pt,sizeof(yaffs_PackedTags2));
	return write(outFile, spare_data, spareSize);
...
int main(int argc, char *argv[])
{
	...
	if(argc < 3)
	{
		...
		printf("           file       the output file to hold the image\n");
