...
{
  TagLib_File *tfile = reinterpret_cast<TagLib_File *>(FileRef::create(filename));
  File *f = reinterpret_cast<File *>(tfile);
  f->setReadOnly(false);

  return tfile;
}

TagLib_File *taglib_file_new_readonly(const char *filename)
{
