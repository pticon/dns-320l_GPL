...
tmp_dir="/home/tmp/${USER}/${PROJECT_NAME}/quota"

#run cmd "sh installfile.sh nas" to cp file to nas
if [ "${1}" = "nas" ]; then
	cp tmp_install/sbin/quotacheck ${TMP_ROOT_FS}/bin
	cp tmp_install/sbin/quotaon ${TMP_ROOT_FS}/bin
	cp tmp_install/sbin/repquota ${TMP_ROOT_FS}/bin
	cp tmp_install/sbin/setquota ${TMP_ROOT_FS}/bin
fi

#cp file to tmp
mkdir -p ${tmp_dir}

cp tmp_install/sbin/quotacheck ${tmp_dir}
cp tmp_install/sbin/quotaon ${tmp_dir}
cp tmp_install/sbin/repquota ${tmp_dir}
cp tmp_install/sbin/setquota ${tmp_dir}
