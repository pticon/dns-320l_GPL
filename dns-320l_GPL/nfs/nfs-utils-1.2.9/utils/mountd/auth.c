		...
		const char *path)
{
	...
	if (path[0] != '/') {
		...
		syslog(LOG_INFO,"NFS requested Path ERROR .IP = %s , Directory = %s",
			   host_ntop(caller, buf, sizeof(buf)), path);
		closelog();
		return exp;
	}
	...
	case unknown_host:
		...
		/* Alpha.tim add : For customize log message .  */
		openlog("NFS",LOG_ODELAY|LOG_PID,LOG_USER);
		syslog(LOG_INFO,"NFS client not be allowed to mount the directory.IP = %s , Directory = %s",
			   host_ntop(caller, buf, sizeof(buf)), path);
		closelog();
		break;
	...
	case not_exported:
		...
		/* Alpha.tim add : For customize log message .  */
		openlog("NFS",LOG_ODELAY|LOG_PID,LOG_USER);
		syslog(LOG_INFO,"NFS requested file or directory not in the exported list.IP = %s , Directory = %s",
			   host_ntop(caller, buf, sizeof(buf)), path);
		closelog();
		break;
	...
	case success:
		...
		if (!strcmp(what ,"mount")){
			openlog("NFS",LOG_ODELAY|LOG_PID,LOG_USER);
			syslog(LOG_INFO,"NFS client mounts device's directory success.IP = %s , Directory = %s",
			       host_ntop(caller, buf, sizeof(buf)), path);
			closelog();
		}
		if (!strcmp(what , "unmount")){
			openlog("NFS",LOG_ODELAY|LOG_PID,LOG_USER);
			syslog(LOG_INFO,"NFS client un-mounts device's directory success.IP = %s , Directory = %s",
				   host_ntop(caller, buf, sizeof(buf)), path);
			closelog();	
		}
		break;
