...
int findNeedRotating(struct logInfo *log, int logNum)
{
	...
	if (log->criterium == ROT_SIZE) {
		strcpy(bufs_2, log->files[logNum]);
		loc = strrchr(bufs_2,'/');
		*loc = 0x0;		
		printf("bufs_2 = %s\n", bufs_2);
		
		sprintf(bufs, "du -b %s", bufs_2);
		fd = popen(bufs,"r");		
		if ( fscanf(fd,"%lld %s",&fSize, bufs)!= EOF ) {
			printf("fSize = %lld %s, log->threshhold = %lld\n", fSize, bufs, log->threshhold);
			state->doRotate = (fSize >= log->threshhold);
		}
		pclose(fd);		
	...
	//by Cycle
	if (state->lastRotated.tm_year != now.tm_year ||
		   state->lastRotated.tm_mon != now.tm_mon) {
		   
		if (log->criterium2 == ROT_MONTHLY) {
		...
		} else if (log->criterium2 == ROT_YEARLY) {
			/* rotate if the logs haven't been rotated this year */
			state->doRotate = (now.tm_year != state->lastRotated.tm_year);
		}
		else if (log->criterium2 ==  ROT_TWO_MONTHLY) {
			/* rotate if the logs haven't been rotated this month or
			   this year */
			if (now.tm_year != state->lastRotated.tm_year) {
				if (abs((now.tm_mon+12) - state->lastRotated.tm_mon) >= 2) {
					state->doRotate = 1;
				}
			} else  if (abs((now.tm_mon) - state->lastRotated.tm_mon) >= 2) {
				state->doRotate = 1;
			}
		}else if (log->criterium2 == ROT_THREE_MONTHLY) {
			/* rotate if the logs haven't been rotated this month or
			   this year */
			if (now.tm_year != state->lastRotated.tm_year) {
				if (abs((now.tm_mon+12) - state->lastRotated.tm_mon) >= 3) {
					state->doRotate = 1;
				}
			} else  if (abs((now.tm_mon) - state->lastRotated.tm_mon) >= 3) {
				state->doRotate = 1;
			}
		} else if (log->criterium2 ==  ROT_SIX_MONTHLY) {
			/* rotate if the logs haven't been rotated this month or
			   this year */
			if (now.tm_year != state->lastRotated.tm_year) {
				if (abs((now.tm_mon+12) - state->lastRotated.tm_mon) >= 6) {
					state->doRotate = 1;
				}
			} else  if (abs((now.tm_mon) - state->lastRotated.tm_mon) >= 6) {
				state->doRotate = 1;
			}
		}
	...
	if (conn != NULL) {
		if (mysql_real_connect(conn, "localhost", "root", "XP4VddgD0zd8IbKQ", "Syslog", 0, NULL, 0) != NULL) { 
			mysql_query(conn,"select count(*) from SystemEvents");
			result = mysql_store_result(conn);
			if (result != NULL) {
				row = mysql_fetch_row(result);
				if (log->criterium3 == ROT_LOGCOUNT) {
					//printf("atoll(row[0]) = %llu\n", atoll(row[0]));
					//printf("log->logCount = %llu\n", log->logCount);
					if (atoll(row[0]) > log->logCount)
						state->doRotate = 1;
				}				
			}
		}
...
#if defined(ALPHA_CUSTOMIZE)
	system("syslog_db_rotate.sh quota_check & sleep 1");
#endif
    }
...
#if defined(ALPHA_CUSTOMIZE)
		system("syslog_db_rotate.sh & sleep 1");
#else
#ifdef WITH_SELINUX
		if (selinux_enabled) {
			security_context_t oldContext = NULL;
...
int rotateLogSet(struct logInfo *log, int force)
{
	...
	break;
#if defined(ALPHA_CUSTOMIZE)
	case ROT_TWO_MONTHLY:
	message(MESS_DEBUG, "Two monthly ");
	break;
	case ROT_THREE_MONTHLY:
	message(MESS_DEBUG, "Three monthly ");
	break;
	case ROT_SIX_MONTHLY:
	message(MESS_DEBUG, "Six monthly ");
	break;
#endif
    }
