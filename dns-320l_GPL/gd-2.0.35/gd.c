/* $Id: gd.c,v 1.1.1.1 2009/10/09 02:34:07 jack Exp $ */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
/* +alpha.zoe 10142011 comment */ 
BGD_DECLARE(void) gdImageCopyRotatedLimited (gdImagePtr dst,
		    gdImagePtr src,
  ...
  int c;
	...
	switch(angle){
		case 90:
			for(dx = 0 ; dx < srcWidth; dx++){
				for(dy = 0; dy < srcHeight; dy++){
					c = gdImageGetPixel (src, dx, srcHeight-dy);
					gdImageSetPixel (dst, dy, dx, c);
				}	
			}
			break;
		case 180:
			for(dx = 0 ; dx < srcHeight; dx++){
				for(dy = 0; dy < srcWidth; dy++){
					c = gdImageGetPixel (src, srcWidth-dy, srcHeight-dx);
					gdImageSetPixel (dst, dy, dx, c);
				}	
			}
			break;	
		case 270:
			for(dx = 0 ; dx < srcWidth; dx++){
				for(dy = 0; dy < srcHeight; dy++){
					c = gdImageGetPixel (src, srcWidth-dx, dy);
					gdImageSetPixel (dst, dy, dx, c);
				}	
			}
			break;
