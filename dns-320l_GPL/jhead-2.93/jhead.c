...
static int CreateGPSInfo = FALSE;
static Gpsinfo_t gpsinfo;
...
void ProcessFile(const char * FileName)
{
	...
	if (CreateGPSInfo){
    	...
    	if( !gpsinfo.GpsLat || !gpsinfo.GpsLong || !gpsinfo.GpsAlt)
    		return;
		...
		if(!ExifSection){
			...
			create_EXIF();
			ExifSection = FindSection(M_EXIF);
			if(!ExifSection) goto badtime;
		}		
		
		if(!exif_set_tag(ExifSection->Data,TAG_GPSINFO,(void *)&gpsinfo)) goto badtime;
...
static void Usage (void)
{
    ...
    printf("Where:\n"
           ...
           "  -mkgps     Set or create new gps info if it is not exist.\n"
...
int main (int argc, char **argv)
{
    ...
    // General metadata options:
        if (!strcmp(arg,"-te")){
            ...
            DoModify |= MODIFY_JPEG;
		}else if (!strcmp(arg,"-mkgps")){
			CreateGPSInfo = TRUE;
			DoModify |= MODIFY_JPEG;
			gpsinfo.GpsLat = argv[++argn];
			gpsinfo.GpsLong = argv[++argn];
			gpsinfo.GpsAlt = argv[++argn];
    	// Output verbosity control
