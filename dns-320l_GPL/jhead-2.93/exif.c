...
static void ProcessExifDir(unsigned char * DirStart, unsigned char * OffsetBase, 
        unsigned ExifLength, int NestingLevel, int myoffset)
{
    int de;
    int a;
    int NumDirEntries;
        ...
        switch(Tag){
            ...
            case TAG_INTEROP_OFFSET:
                if (Tag == TAG_INTEROP_OFFSET && ShowTags) printf("%s    Interop Dir:",IndentString);
                {
                    ...
                    if(myoffset){
                    	setoffset = (Get32u(ValuePtr)+myoffset);
                    	Put32u(ValuePtr,setoffset);
                    }
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
                        ErrNonfatal("Illegal Exif or interop ofset directory link",0,0);
                    }else{
                        ProcessExifDir(SubdirStart, OffsetBase, ExifLength, NestingLevel+1, myoffset);
                    }
                    continue;
                }
                break;
            ...
            if (Offset){
                ...
                if (SubdirStart > OffsetBase+ExifLength || SubdirStart < OffsetBase){
                    ...
                    if (SubdirStart <= OffsetBase+ExifLength){
                        ...
                        if(myoffset){
                    		setoffset = (Offset+myoffset);
                    		Put32u(DirStart+2+12*NumDirEntries,setoffset);
                    	}
                        ProcessExifDir(SubdirStart, OffsetBase, ExifLength, NestingLevel+1, myoffset);
                    }
                }
...
//--------------------------------------------------------------------------
int exif_insert_tag(unsigned char *ExifSection,int tag)
{
	int NumDirEntries;
	int DataWriteIndex;
	int DataReadIndex;
	...
	Size = (*ExifSection << 8)|(*(ExifSection+1));
	DirStart = ExifSection+8+Get32u(ExifSection+12);
	NumDirEntries = Get16u(DirStart);
	Header_Size = (DirStart-ExifSection);
	
	NewBuf = malloc(Size + 512);
	if (NewBuf == NULL){
    ...
    // 0. Copy EXIF header
    memcpy(NewBuf, ExifSection, Header_Size);
    DataWriteIndex += Header_Size;
    DataReadIndex += Header_Size;
    
    // 1. Copy EXIF entry
    ProcessExifDir(DirStart, ExifSection+8, Size, 0, 12);
    memcpy(NewBuf+DataWriteIndex,DirStart,2+NumDirEntries*12);
    Put16u(NewBuf+DataWriteIndex, (NumDirEntries+1)); 
    DataWriteIndex += 2 + NumDirEntries*12;
    DataReadIndex += 2 + NumDirEntries*12;
    DirContinuation = Get32s(ExifSection+DataReadIndex);
    DataReadIndex += 4; 
    //printf("DirContinuation = %x\n",DirContinuation);
   
    // 2. Create New entry
    DataIndex = Size+4;
	switch(tag){
		case TAG_GPSINFO:
			Put16u(NewBuf+DataWriteIndex, TAG_GPSINFO);          // Tag
            Put16u(NewBuf+DataWriteIndex + 2, FMT_ULONG);        // Format
            Put32u(NewBuf+DataWriteIndex + 4, 1);                // Components
            Put32u(NewBuf+DataWriteIndex + 8, DataIndex); 		 // Pointer or value.
            //Put32u(NewBuf+DataWriteIndex + 8, 0);
            DataWriteIndex += 12;
			
			Put32u(NewBuf+DataWriteIndex, DirContinuation);
			DataWriteIndex += 4;
			break;
	...
	// 3. Copy EXIF content
	memcpy(NewBuf+DataWriteIndex, (ExifSection+DataReadIndex), (Size-DataReadIndex));
    DataWriteIndex += (Size-DataReadIndex);  
	DataReadIndex += (Size-DataReadIndex); 
	...
	switch(tag){
		case TAG_GPSINFO:
			// GPS dir entry
			DataWriteIndex += create_gpsinfo((NewBuf+DataWriteIndex), DataWriteIndex);
			break;
	...
	NewBuf[0] = (unsigned char)(DataWriteIndex >> 8);
    NewBuf[1] = (unsigned char)DataWriteIndex;
	...
	{
		FILE * fp;
		fp = fopen("/mnt/HD/HD_a2/exif.dat","wb");
		if(fp){
			fwrite(NewBuf,DataWriteIndex,1,fp);
		}
		fclose(fp);
	}
	free(NewBuf);
...
int exif_set_tag(unsigned char *ExifSection,int tag,void *param)
{
	...
	entry = exif_find_tag(DirStart,ExifSection+8,tag );
	if(!entry){	
		Section_t *NewSection;
		exif_insert_tag(ExifSection,tag);
		NewSection = FindSection(M_EXIF);
		if(!NewSection)
			return FALSE;
		
		ExifSection = NewSection->Data;	
		DirStart = ExifSection+8+Get32u(ExifSection+12);			
		entry = exif_find_tag(DirStart,ExifSection+8,tag );
		if(!entry){
			//printf("no entry...\n");
	...
	switch(tag){
		case TAG_GPSINFO:
				...
				//		printf("%x ",*(entry+i));
				//}	
				SetGpsInfo( entry, (Gpsinfo_t *)param, ExifSection+8 );
        		break;
        	default:
        		break;
	}
...
void process_EXIF (unsigned char * ExifSection, unsigned int length)
{
    ...
    ProcessExifDir(ExifSection+8+FirstOffset, ExifSection+8, length-8, 0, 0);
...
void create_EXIF(void)
{
...

    {
        ...
        // Enitries go here...
        {
            ...
            Put32u(Buffer+DirIndex + 8, DataWriteIndex-8); // Pointer or value.
            DirIndex += 12;
            exif_private_len = (2 + 1*12 + 4 + 20);
            
            // Link to GPS dir entry
            Put16u(Buffer+DirIndex, TAG_GPSINFO);          // Tag
            Put16u(Buffer+DirIndex + 2, FMT_ULONG);        // Format
            Put32u(Buffer+DirIndex + 4, 1);                // Components
            Put32u(Buffer+DirIndex + 8, DataWriteIndex+exif_private_len-8); // Pointer or value.
            DirIndex += 12;
