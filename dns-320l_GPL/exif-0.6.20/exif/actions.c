...
void action_save (ExifData *ed, ExifLog *log, ExifParams p, const char *fout)
{
	JPEGData *jdata;
...
#if 1  //+Bing test
	jpeg_data_load_file (jdata, fout);
#else
	jpeg_data_load_file (jdata, output_file);
#endif
	...
	fprintf (stdout, _("1 Wrote file '%s'."), fout);
	fprintf (stdout, "\n");
#else
	jpeg_data_save_file (jdata, output_file);
	jpeg_data_unref (jdata);

	fprintf (stdout, _("Wrote file '%s'."), output_file);
	fprintf (stdout, "\n");
#endif
}
...
action_save_thumb (ExifData *ed, ExifLog *log, ExifParams p, const char *fout)
{
	...
	if (!f)
		...
		fclose (f);
		fprintf (stdout, _("2 Wrote file '%s'."), fout);
		fprintf (stdout, "\n");
	}
}
