...
LOG(log_error, logtype_uams, "PAM DHX2: libgcrypt versions GCRYPT_VERSION: %s", GCRYPT_VERSION);
    if (bits < 256)
        qbits = bits / 2;
    else
        qbits = (bits / 40) + 105;
...
LOG(log_error, logtype_uams, "success");
    return 0;

error:
    gcry_prime_release_factors(factors);

LOG(log_error, logtype_uams, "failed");
    return result;
}
...
static int uam_setup(const char *path)
{
    ...
    LOG(log_debug, logtype_uams, "DHX2: generating mersenne primes, path: %s", path);
    /* Generate p and g for DH */
    if (dh_params_generate(PRIMEBITS) != 0) {
        LOG(log_error, logtype_uams, "DHX2: Couldn't generate p and g");
        return -1;
    }
    LOG(log_debug, logtype_uams, "DHX2: generating mersenne primes done");
