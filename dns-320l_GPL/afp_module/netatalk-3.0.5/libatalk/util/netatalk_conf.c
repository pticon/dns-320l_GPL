                            ...
                            const char *preset)
{
	...
	if (pwd && strstr(path_in, "/.!@#$recycle")){
			strlcpy(path, path_in, MAXPATHLEN);
			strlcat(path, "/", MAXPATHLEN);
			strlcat(path, pwd->pw_name, MAXPATHLEN);
	}
	else{
    	strlcpy(path, path_in, MAXPATHLEN);
	}
...
#if (defined ALPHA_CUSTOMIZE) && (defined PROJECT_FEATURE_AFP_RECYCLE_BIN)
    if ((val = getoption(obj->iniconfig, section, "recycle bin", preset, NULL)))
        volume->v_recycle_bin = 1;
#endif
    ...
    /* let RW > RO	alpha.Lock++ 2013/10/03
     * Handle read-only behaviour. semantics:
     * 1) neither the rolist nor the rwlist exist -> rw
     * 2) user in rolist & not in/exists rwlist -> ro.
     * 3) both not in rwlist and rolist -> rw.
     * 4) cnid scheme = last -> ro forcibly.
     */
	 
    if (pwd) {
        if (accessvol(obj, getoption(obj->iniconfig, section, "rolist", preset, NULL), pwd->pw_name) == 1
            && accessvol(obj, getoption(obj->iniconfig, section, "rwlist", preset, NULL), pwd->pw_name) != 1)
            volume->v_flags |= AFPVOL_RO;
    }
