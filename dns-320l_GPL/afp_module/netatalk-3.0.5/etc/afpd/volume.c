...
#ifdef ALPHA_CUSTOMIZE  // alpha.Hwalock++ 2014/1/3
#include <libaatalk.h>	// alpha atalk lib
#endif
...
static int volume_openDB(const AFPObj *obj, struct vol *volume)
{
...
#ifdef ALPHA_CUSTOMIZE	//alpha.Hwalock++ 2014/1/3
	a_chkDB_r = alpha_chk_DB(volume->v_dbpath, (void *)volume->v_cdb, a_chkDB_msg);
	switch(a_chkDB_r){
	case A_ATALK_CANNOT_STATFS:
		LOG(log_error, logtype_afpd, "Cannot find volume: %s, volume_openDB return failed", volume->v_dbpath);
		return A_ATALK_CANNOT_STATFS;
	case A_ATALK_OUTOF_SPACE:
		LOG(log_error, logtype_afpd, "No more space to create CNID DB on %s", volume->v_dbpath);
		break;
	default:	
#endif
    ...
    if ( ! volume->v_cdb && ! (flags & CNID_FLAG_MEMORY)) {
		...
		if (a_chkDB_r == A_ATALK_OUTOF_SPACE)
			setmessage(a_chkDB_msg);
		else
#endif
