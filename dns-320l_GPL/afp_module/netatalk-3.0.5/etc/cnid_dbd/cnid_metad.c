...
#ifndef ALPHA_CUSTOMIZE	//alpha.Hwalock +++ forgot when
...
#else	
		/* for ALPHA cbd to dbd */	
		cmd_argv[0] = "rm";
		cmd_argv[1] = "-rf";
		cmd_argv[2] = bdata(oldpath);
		cmd_argv[3] = NULL;
		if (run_cmd("rm", cmd_argv) != 0) {
			LOG(log_error, logtype_cnid, "set_dbdir: deleting CNID db in \"%s\" failed: %s", bdata(oldpath), strerror(errno));
		}
