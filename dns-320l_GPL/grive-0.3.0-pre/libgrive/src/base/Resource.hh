...
private :
	...
	void SyncSelf( Syncer* syncer, const Val& options ) ;
    	...
    	void createHIddenFile(fs::path path, bool isNew) ;
	void createHIddenFile(fs::path path, bool isNew, int twoWayDirection) ;

	std::string getHiddenFileExt(fs::path path);

	void DeleteHiddenFiles(fs::path path);

	std::string BuildHiddenFilePath(fs::path path, std::string ext);
