...
void Syncer1::GetAbout()
{
	http::XmlResponse xml ;
	
	Log( "Getting abouts", log::info ) ;
	m_http->Get( feed_metadata, &xml, http::Header() ) ;
	
	std::cout<< xml.Response()["gd:quotaBytesUsed"].front().Value().c_str() << std::endl ;
	std::cout<< xml.Response()["gd:quotaBytesTotal"].front().Value().c_str() << std::endl ;
}
