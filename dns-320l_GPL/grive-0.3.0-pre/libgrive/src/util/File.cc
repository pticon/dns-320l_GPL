...
File::File( const fs::path& path, bool largeFile ) : m_fd( -1 )
{
...
off_t File::Seek( off_t offset, int whence, bool largeFile )
{
	m_largeFile = largeFile;
	m_offset = offset;
...
u64_t File::Size(bool largeFile)
{
	assert( IsOpened() ) ;
	m_largeFile = largeFile;
	struct stat s = FStat(m_fd) ;
	...
	u64_t size = 0;
	if(m_largeFile == true)
		size =  static_cast<uint64_t>( s.st_size - m_offset) ;
	else
		size = static_cast<uint64_t>( s.st_size) ;
