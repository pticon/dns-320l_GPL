...
std::string MD5::Get2( const fs::path& file )
{
	int ret = -1;
	char cmd[256] = {0};
	char md5_cmd[1024] = {0};
	...
	sprintf(md5_cmd, "md5sum '%s' | cut -d ' ' -f1", file.string().c_str());
    	//Log( "md5_cmd = %1%, ", md5_cmd, log::info ) ;
	if((fp = popen(md5_cmd, "r")))
	{
		ret = -1;
		...
		if(ret < 0)
		{
			Log( "md5 error: null", log::info ) ;
			return "md5 error: null";
		}
	}
