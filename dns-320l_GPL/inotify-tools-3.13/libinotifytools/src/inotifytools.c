...
int inotifytools_remove_watch_recursive_by_filename( char const * filename) {
	...
	all = rbopenlist(tree_wd);
	if(all != NULL)
	{
		p = rbreadlist(all);
		while (p) {
			if(strncmp(filename, p->filename, strlen(filename)) == 0 && 
			    (strlen(filename) != strlen(p->filename))) {
				if (remove_inotify_watch(p)) {
					rbdelete(p, tree_wd);
					rbdelete(p, tree_filename);
					destroy_watch(p);
				}
			}
			p = rbreadlist(all);
	...
	/* Remove self at last. Prevent the filename pointer is from the node */
	if (!remove_inotify_watch(w)) return 0;
	rbdelete(w, tree_wd);
	rbdelete(w, tree_filename);
	destroy_watch(w);
...
#endif /* end of ALPHA_CUSTOMIZE_INOTIFYTOOLS */
