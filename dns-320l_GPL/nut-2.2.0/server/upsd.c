...
static int	reload_flag = 0, exit_flag = 0,write_log = 0;
static int lb_happen_time = 0;///*for_ups_ob_shut*/,ob_time = 0,ob_time_tmp = 0;
char tsc[2][16] = {"battery.charge","ups.status"};//,"ups.temperature"};
static int ups_shutflag = 0;
#ifdef	HAVE_IPV6
...
static void mainloop(void)
{
	...
	int lb_now_time = 0;
	int w_filecheck = 0;
	//int ob_shut_time = 0;//for_ups_ob_shut
	ctype_t	*tmpcli, *tmpnext;
	...
	if (res > 0) {
		...
		while (utmp) {
			...
			if(write_log == 1)
			{
				...
				lala = sstate_getinfo(utmp, tsc[0]);
				if((lala = sstate_getinfo(utmp,tsc[0])) != NULL)
				{
					fp = fopen("/tmp/ups_state","w");
					for(count = 0;count < 2;count ++)
					{
						lala = sstate_getinfo(utmp, tsc[count]);
						sprintf(bufs,"%s %s\n",tsc[count], lala);
						fputs(bufs,fp);
					}
					fclose(fp);
				...
				else if ((lala = sstate_getinfo(utmp,tsc[1])) != NULL)
				{
					fp = fopen("/tmp/ups_state","w");
					sprintf(bufs,"%s Unknown\n",tsc[0]);
					fputs(bufs,fp);
					lala = sstate_getinfo(utmp, tsc[1]);
					sprintf(bufs,"%s %s\n",tsc[1], lala);
					fputs(bufs,fp);
					fclose(fp);
					write_log = 0;
			...
			luko = sstate_getinfo(utmp,"ups.status");
			...
			if(luko != NULL)
			{
				if(strstr(luko,"OL")){
					...
					if((w_filecheck = stat("/tmp/UPS_OB",&sta)) >= 0){
						system("rm /tmp/UPS_OB");
					}
#if 0
					//Let upsmon to do what it need to do .
					if(ups_shutflag == 1){
						system("do_reboot");
						ups_shutflag = 0;
					}
				...
				// if(strstr(luko,"OB")){
					// if(ob_time != 0){
					    // if(ob_time_tmp == 0){
							// ob_time_tmp = time((time_t*)NULL);
						// }
						// ob_shut_time = time((time_t*)NULL);
						// if((ob_shut_time - ob_time_tmp) >= ob_time){
...
static void help(const char *progname) 
{
	...
	printf("  -L		while driver up ,get UPS info with this , show on /tmp/ups_state\n");
	printf("  -f		stay in the foreground for testing\n");
	printf("  -h		display this help\n");
	printf("  -r <dir>	chroots to <dir>\n");
	printf("  -u <user>	switch to <user> (if started as root)\n");
	printf("  -V		display the version of this software\n");
	//printf("  -X		For UPS On Battery Shut Time set (Mark now,for test)\n");//for_ups_ob_shut
#ifdef	HAVE_IPV6
	printf("  -4		IPv4 only\n");
	printf("  -6		IPv6 only\n");
#endif
...
//for_ups_ob_shut
// static void shuttime(int sig)
// {
	// FILE *fp;
	// char buf[20];
	// if((fp = fopen("/tmp/upsfile","r")) != NULL)
	// {
		// fgets(buf,20,fp);
		// ob_time = atoi(buf);
		// fclose(fp);
...
int main(int argc, char **argv)
{
	...
	while ((i = getopt(argc, argv, "+h46p:r:i:fu:X:LVc:D")) != -1) {
		switch (i) {
			case 'h':
				help(progname);
				break;
