...
static int strict_allocate_ftruncate(vfs_handle_struct *handle, files_struct *fsp, SMB_OFF_T len)
{
	...
	/* nelson 20150902+ fix the bug that file block size is greater than file real size
	   if strict allocate is enabled.
	 */
	//if (st.st_ex_size > len)
	if (st.st_ex_size > len || st.st_ex_blocks * 512 > len)
