...
/* Issue a diagnostic and stop.  Be a little paranoid about recursion. */
    ...
    //printf("NTP Server failure!!!\n");	//+Wilson05252006
    //system("touch /tmp/sntp_ng");	//+Wilson05252006
    exit(EXIT_FAILURE);
}
...
#if 0	//alpha.Bing modified 12132005
    if (absoff > prompt) {
...
#endif	//alpha.Bing modified 12132005+
...
/* Tidy up the socket, issues diagnostics and perform the action. */
    ...
    //alpha.tim add 2009/06/03
    //xml_set_tmp_str("/stime/status","done");
    //xml_write_file(CONFIG_FILE_PATH); 
	//alpha.tim add end
...
/* Check the arguments for consistency and set the defaults. */
    ...
    if (action == action_query) {
       ...
       if (argc > 1) {
            ...
            if ( ( !strcmp(hostnames[0],"1.fsc.pool.ntp.org")) || 
            	( !strcmp(hostnames[0],"2.fsc.pool.ntp.org")) || 
            	( !strcmp(hostnames[0],"3.fsc.pool.ntp.org")) )
            {
            	clear_alarm_flag == 0;
            	memset(buf , 0 , 80 );
            	system("rm -f /tmp/pingtmp");
            	sprintf(buf , "ping -c 2 %s > /tmp/pingtmp",hostnames[0]);
            	system(buf);
            	if  ( (fp = fopen("/tmp/pingtmp","r")) != NULL )
            	{
            		fgets(buf , 80 , fp );
            		fclose(fp);
            		p1 = strchr(buf , 0x28);
