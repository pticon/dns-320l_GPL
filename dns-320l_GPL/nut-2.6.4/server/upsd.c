	...
	for (i = 0; i < nfds; i++) {
		...
		if (fds[i].revents & POLLIN) {
			...
			case DRIVER:
				...
				if (write_log == 1) {
					if ((lala = sstate_getinfo((upstype_t *)handler[i].data, tsc[0])) != NULL) {
#if defined(LIGHTNING_4A)
						lala = sstate_getinfo((upstype_t *)handler[i].data, tsc[1]);
						if (strstr(lala , "OL")) {
							sprintf(bufs , "ups_info -b \"%s\" -s \"OL\"",sstate_getinfo((upstype_t *)handler[i].data, tsc[0]));	
						}
						else if (strstr(lala , "LB")) {
							sprintf(bufs , "ups_info -b \"%s\" -s \"LB\"",sstate_getinfo((upstype_t *)handler[i].data, tsc[0]));		
						}
						else if (strstr(lala , "OB")) {
						...
						for (count = 0 ; count < 2 ; count ++) {
							lala = sstate_getinfo((upstype_t *)handler[i].data, tsc[count]);
					...
					else if ((lala = sstate_getinfo((upstype_t *)handler[i].data,tsc[1])) != NULL) {
#if defined(LIGHTNING_4A)
						lala = sstate_getinfo((upstype_t *)handler[i].data, tsc[1]);
						if (strstr(lala , "OL")) {
				...
				if ((luko = sstate_getinfo((upstype_t *)handler[i].data , "ups.status")) != NULL) {
					if (strstr(luko , "OL")) {
						lb_happen_time = 0 ;
						lb_now_time = 0 ;
					...
					if (strstr(luko , "OB")) {
						...
						#if defined(LIGHTNING_4A)
						if ((lulu = sstate_getinfo((upstype_t *)handler[i].data, tsc[0])) != NULL) {
							if (send_to_up == 0) {
								send_to_up = 1 ;
								memset(bufs , 0 , sizeof(bufs));
								strncpy(bufs , lulu , strlen(lulu));
								fireAlert(1, 31 , bufs);
								//sprintf(bufs , "up_send_ctl ErrStatus 0031 1 %s",lulu);
								//system(bufs);
								now_battery_status = atoi(lulu);
							}
							else {
								if (now_battery_status != atoi(lulu)) {
									memset(bufs , 0 , sizeof(bufs));
									strncpy(bufs , lulu , strlen(lulu));
									fireAlert(1, 31 , bufs);
									//memset(bufs , 0 , sizeof(bufs));
