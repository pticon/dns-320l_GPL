...
//						
//		tmp_load_cmd=avahi_strdup_printf("(rm -rf %s)",AVAHI_SERVICE_ALPHA_TMP_DIR);
//			     				
//   	system(tmp_load_cmd);
//   	avahi_free(tmp_load_cmd);
//   	
//   	mkdir(AVAHI_SERVICE_ALPHA_TMP_DIR,0600);
...
//		if(access(AVAHI_SERVICE_ALPHA_TMP_DIR,F_OK))
...
//    	fclose(fopen(AVAHI_SERVICE_ALPHA_AVAHI_KILL,"w"));
    	
        if (daemon_pid_file_kill_wait(SIGTERM, 60) < 0) {
