/*
 * $Id: xml-rpc.c,v 1.1 2010/02/25 01:40:32 curtis Exp $
 *
...
#include "webserver.h"
#include "xml-rpc.h"
/* add for path error msg curtis@alpha 02_10_2010 */
#include "util.h"
/* typedefs */
...
void xml_update_config(WS_CONNINFO *pwsc) {
    ...
    /* add this for detailed curtis@alpha 02_11_2010 */
    char **valuearray;
		int index;
		int path_err =0;
		int path_len =0;
		int err_index= 0;
		char *err_msg=", Please check following music folder: ";
		int err_msg_len=0; 
    ...
    while((handle=ws_enum_var(pwsc,&arg,&value,handle)) != NULL) {
        ...
        if(ptmp) {
            *ptmp++ = '\0';
            ...
            /* when error msg is mp3_dir, output error reason. curtis@alpha 01/14/2010 */
						/* err = 3 */
            ...
            if(err != CONF_E_SUCCESS) {
                ...
                has_error = TRUE;
                    		...
                    		printf("err_msg_len=%d\n",err_msg_len);
                    		
                    		
                    		printf("valuearray[%d]=%s\n",err_index,valuearray[err_index]);
                    		path_len=strlen(valuearray[err_index]);
                    		
                    		printf("err_msg_len=%d\n",err_msg_len);
                    		printf("path_len=%d\n",path_len);
                    		badparms_len = path_len+err_msg_len;
                    		badparms = (char*)malloc(badparms_len+1);
                    		
                    		strcpy(badparms,err_msg);
                    		printf("badparms=%s\n",badparms);
                    		printf("valuearray[err_index]=%s\n",valuearray[err_index]);
                    		strcat(badparms, valuearray[err_index]);
...
void xml_get_stats(WS_CONNINFO *pwsc) {
    ...
    xml_pop(pxml); /* stat */
   	...
   	#if 0
    xml_push(pxml,"stat");
    xml_output(pxml,"name","DB Version");
    xml_output(pxml,"value","%d",db_revision());
    xml_pop(pxml); /* stat */
		#endif
