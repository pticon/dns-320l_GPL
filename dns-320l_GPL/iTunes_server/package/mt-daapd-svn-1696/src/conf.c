/*
 * $Id: conf.c,v 1.1 2010/02/25 01:40:31 curtis Exp $
...
int _conf_verify_element(char *section, char *key, char *value) {
    ...
    case CONF_T_MULTIPATH:
    		//curtis@alpha 02_10_2010
        if(util_split(value,",",&valuearray) >= 0) {
            index = 0;
            transform_volume(valuearray);
            while(valuearray[index]) {
            		printf("valuearray[%d]=%s\n",index, valuearray[index]);
            		printf("len of valuearray=%d\n", strlen(valuearray[index]));
            		printf("NAS_HD_PATH=%s\n", NAS_HD_PATH);
...
int conf_set_string(char *section, char *key, char *value, int verify) {
    ...
    //for mp3_dir curtis@alpha
		int len = 0;
		char tmp[1];
		char *sym=",";
		
    util_mutex_lock(l_conf);
printf("conf.c: conf_set_string():section=%s, key=%s, value=%s\n",section,key,value);
		#if 0
    /* verify the item */
    /* curtis@alpha 06_24_2009 dont't check the real_path of mp3dir */
		
		
		if( !strcmp(key, "mp3_dir") )	{
			printf("conf.c.c: conf_set_string():this config tag is mp3dir\n");
			
			/* curtis@alpha check music path 07_02_2009 */
			if( strncmp(value, "Volume", 6) == 0 && value[8] == 47 ) {/* confirm the head of string is Volume */
	
				if ( strstr(value, ",,") != NULL ) {/* ignore empty path */
					printf("conf.c.c: conf_set_string():ignore empty path\n");
					util_mutex_unlock(l_conf);
					return ERR_EMPTY;	
				}
				
				/* when config only have 1 empty folder curtis@alpha */
				len = strlen(value);
				printf("len=%d\n",len);
				printf("value[0]=%c\n",value[len-1]);
				tmp[0]=value[len-1];
				...
				printf("conf.c.c: conf_set_string():this config tag isn't Volume\n");
				util_mutex_unlock(l_conf);
				return ERR_VOL;
			}
