/*
 * $Id: mp3-scanner.c,v 1.1 2010/02/25 01:40:32 curtis Exp $
...
void scan_music_file(char *path, char *fname,
    ...
    if(scan_get_info(mp3file.path,&mp3file)) {
        if(is_compdir)
    		 ...
    		 *
    		 */
    		DBG_FILE_SCAN(printf("mp3-scanner.c: scan_get_info(): convert_info\n");)

				//printf("mp3-scanner: scan_music_file(): code_page=%s\n",code_page);
				if (!unicode16_text)
    		charset_convertor(code_page, &mp3file);
    		//convert_charset_main(&mp3file, code_page);
