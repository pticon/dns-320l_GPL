 ...
 * curtis@alpha 06_23_2009
 */
void transform_volume(char **path){
	
		int dir_index=0;
		/* define max length of Mp3Path as 128 */
		char mp3path[512]="/mnt/";
		char mp3_tmp[512]="/mnt/";
		char volume[12];
		char mnt[12];
		char sym[4];
			...
			DBG_PATH_SCAN(printf("main.c: fscanf: /etc/shared_name: %s %s %s\n", volume, sym, mnt);)
			 ...
			 while(dir_index < 1000) {/* break this loop when read last mp3_path */
			  if( path[dir_index] != NULL) {
			  	...
			  	win_to_linux(path[dir_index]);
			  	
					DBG_PATH_SCAN(printf("config_file_path[%d]=%s\n",dir_index,path[dir_index]);)

			   	/* curtis@alpha catch mp3_dir path 06_24_2009 */
			   	char *pstr;
					if( !strncmp(path[dir_index], "/Volume_1", 9) && strlen(volume_1)){  
				  	strcat(mp3path, volume_1);
				  	DBG_PATH_SCAN(printf("Volume_1 and mp3path=%s\n", mp3path);)
				  ...
				  else if ( !strncmp(path[dir_index], "/Volume_2", 9) && strlen(volume_2) ){   
					...
					else if ( !strncmp(path[dir_index], "/Volume_3", 9) && strlen(volume_3)){  					  	
					...
					else if ( !strncmp(path[dir_index], "/Volume_4", 9) && strlen(volume_4)) { 
