/*
 * $Id: webserver.c,v 1.1 2010/02/25 01:40:32 curtis Exp $
 * Webserver library
 *
...
int ws_addarg(ARGLIST *root, char *key, char *fmt, ...) {
    ...
    while(current) {
        if(!strcasecmp(current->key,key)) {
            ...
            free(pnew);
            WS_EXIT();
            //01/07/2010 alpha@curtis for IE8
	    			return TRUE;
...
int ws_copyfile(WS_CONNINFO *pwsc, IOHANDLE hfile, uint64_t *bytes_copied) {
    ...
    while(io_read(hfile,buf,&bytes_read) && bytes_read) {
	    ...
	    //2009_12_10 curtis@alpha for IE8 issue
	    printf("webserver.c: check bytes_cpoied\n");
	    if(bytes_copied)	
