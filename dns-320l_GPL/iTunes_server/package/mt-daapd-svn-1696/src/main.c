/*
 * $Id: main.c,v 1.1 2010/02/25 01:40:32 curtis Exp $
...
int main(int argc, char *argv[]) {
    ...
    free(db_parms);
		...
		/* check ff_ready.txt curtis@alpha 07_29_2009 */
		system("/opt/firefly/sbin/check_ff_ready.sh");
    ...
    if(conf_get_array("general","mp3_dir",&mp3_dir_array)) {
    	...
    	printf("main.c: conf_get_array: Initializing database\n");
    	transform_volume(mp3_dir_array);
    ...
    DPRINTF(E_LOG,L_MAIN,"Serving %d songs.  Startup complete in %d seconds\n",
		...
		/* after initialize database create a file for UI can confirm ff ready  curtis@alpha 07_29_2009 */
		
		printf("main.c: create a ff_ready.txt\n");
		FILE *ff_file = NULL;
		char *file="/mnt/HD_a4/.systemfile/ff_ready.txt";
		ff_file = fopen(file, "w+");
		fclose(ff_file);
