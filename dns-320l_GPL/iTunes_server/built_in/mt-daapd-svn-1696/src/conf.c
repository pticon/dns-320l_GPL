...
int conf_set_string(char *section, char *key, char *value, int verify) {
    ...
    /* curtis@alpha 06_24_2009 dont't check the real_path of mp3dir */
		...
		if( !strcmp(key, "mp3_dir") )	{
			...
			printf("conf.c: volume=%s\n",value);
						
			printf("conf.c: volume[0]=%c\n",value[0]);
			printf("conf.c: volume[1]=%c\n",value[1]);
			printf("conf.c: volume[2]=%c\n",value[2]);
                        printf("conf.c: volume[7]=%c\n",value[7]);
			printf("conf.c: volume[8]=%c\n",value[8]);
...
//modify by ibuki 2013/06/06  start
//    while(*retval) {
//        *retval = tolower(*retval);
//        retval++;
//    }
//modify by ibuki 2013/06/06 end
