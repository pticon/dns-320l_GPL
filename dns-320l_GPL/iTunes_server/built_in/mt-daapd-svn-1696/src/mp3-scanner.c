...
static TAGHANDLER taghandlers[] = {
    ...
    { "mp4a", scan_get_aacinfo, "m4a", "mp4a", 0, "AAC audio file" },
...
int scan_init(char **patharray) {
    ...
    char resolved_path[PATH_MAX];
		...
		mp3_counter=0;
		LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_COUNTER,ITUNES_OK,        
			  												MIPC_TIME_ALWAYS,(int)mp3_counter);
...
int scan_path(char *path, char *code_page) {
    ...
    follow_symlinks = conf_get_int("scanning","follow_symlinks",1);
    			...
    			extensions = strdup(CONF_DEFAULT_EXTENSIONS);
    	#else
...
void scan_filename(char *path, int compdir, char *extensions, char *code_page) {
    ...
    char *ext_end=NULL; //for distinguish  between the supported extension "mp4" and "mp4a"
    int mod_time;
    MP3FILE *pmp3;
    ...
    if(!all_ext) {
	    	#if defined(PROJECT_FEATURE_CUSTOM_WD)
	    			all_ext=strdup(CONF_DEFAULT_EXTENSIONS);
	    	#else
        ...
        if(strlen(fname) > 2) {
            ...
            if(ext && ((int)strlen(ext) > 1)) {
                if(strcasecmp(".m3u",ext) == 0) {
                    ...
                    scan_add_playlistlist(mp3_path);
                } else if((ext_end=strcasestr(all_ext, ext))!=NULL) {
                	ext_end=ext_end+strlen(ext);
                	if((*ext_end)==','|| (*ext_end)=='\0'|| (*ext_end)==' '){
