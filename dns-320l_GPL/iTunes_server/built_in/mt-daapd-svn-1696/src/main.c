...
int filter(const struct dirent *entry)
{
	...
	if(strstr(entry->d_name, ".") == entry->d_name)
		...
		(last_dot = strrchr(entry->d_name, '.')) &&
		(strcasestr(filter_extensions, last_dot)))
		return 1;
	else
		return 0;
}
...
//--------------------------------------------------------------------------
// ROUTINE NAME - add_path
//--------------------------------------------------------------------------
// FUNCTION: static int add_path(char *path)
//
// DESCRIPTION: add new path for mp3_dir and scan the database
//
// INPUT: char *path
//
// OUTPUT: 
...
//			< 0 	FAIL
//--------------------------------------------------------------------------
static int add_path(char *configfile, char *path) 
{
	...
	DPRINTF(E_DBG,L_SCAN,"Starting add_path %s %s\n",path,configfile);    
...
//		return -1;
//
//	out_fp =  fopen("/tmp/mt-daapd.conf","w+");
//	if ( out_fp == NULL ){
//		fclose(fp);
//		return -1;
//	}
//
//	while (fgets(resolved_path, PATH_MAX, fp)) {
//		if(strstr(resolved_path,"mp3_dir")){
...
//				else
//					sprintf(p,",%s\n",path);
//				fputs(resolved_path,out_fp);
//			}else{
//				// has the path already
//				fclose(fp);
//				fclose(out_fp);
//				remove("/tmp/mt-daapd.conf");
//				return 0;
//			}
//		}else{
//			fputs(resolved_path,out_fp);
...
//	fclose(out_fp);
//
//	rename("/tmp/mt-daapd.conf",configfile);
	
	// scan into database
	conversion_codepage = conf_alloc_string("scanning","mp3_tag_codepage","ISO-8859-1");
...
static int remove_path(char *configfile,char *path,char **pe) 
{
	char resolved_path[PATH_MAX];
	FILE *fp,*out_fp;
	char *p,*pdot;
	...
	DPRINTF(E_DBG,L_SCAN,"Starting remove_path %s %s\n",path,configfile); 
...
//	fclose(out_fp);
//
//	err = rename("/tmp/mt-daapd.conf",configfile);
	
	// delete from database
	realpath(path,resolved_path);
	db_delete_path(pe,resolved_path);
...
int main(int argc, char *argv[]) {
	...
	unlink("/tmp/ff_ready.txt");

    while((option=getopt(argc,argv,"D:d:c:P:mfrysiuvab:A:R:Vk")) != -1) {
        switch(option) {
        case 'a':
            appdir = 1;
            break;
		...
		// zoe 20131209 start           
		case 'A':
			a_path = strdup(optarg);
			reset_IPC=0;
			break;
		case 'R':
			r_path = strdup(optarg);
			reset_IPC=0;
			break;	
		// zoe 20131209 end	
        default:
	...
	if(reset_IPC){
	    LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_COUNTER,ITUNES_OK, MIPC_TIME_ALWAYS,(int)clean_IPC);
	    LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_TOTAL,ITUNES_OK, MIPC_TIME_ALWAYS,(int)clean_IPC);
	    LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_SERVER_READY,ITUNES_OK, MIPC_TIME_ALWAYS,(int)server_ready);

			 LIB_Get_IPC_int(MIPC_ITUNES,ITUNES_SERVER_READY, &server_ready);
	     //printf("ITUNES_SERVER_READY = %d\n", server_ready);
    }
    ...
    // zoe 20131209 start 
	if(a_path){
		err = add_path(configfile,a_path);
		if(err < 0)
			exit(EXIT_FAILURE);
		exit(EXIT_SUCCESS);	
	}
	...
	if(r_path){
		err = remove_path(configfile,r_path,&perr);
		if(err < 0)
			exit(EXIT_FAILURE);
		exit(EXIT_SUCCESS);	
	}
    ...
    if(!song_count){
    	/* compute total number of mp3 */
    			...
    			filter_extensions=strdup(CONF_DEFAULT_EXTENSIONS);
    	#else
    			filter_extensions=conf_alloc_string("general","extensions",".mp3,.m4a,.m4p");
    	#endif
			conf_get_array("general","mp3_dir",&mp3_dir_array);
			ind=0;
			while((mp3_dir_array[ind] != NULL) && (!util_must_exit())) {
	    	  		if (strlen(mp3_dir_array[ind]) >4 ){// /mnt/HD/
					total_mp3 += get_mp3_count(mp3_dir_array[ind]);
				}
				ind++;
			}

			free(filter_extensions);
			printf("**************** total mp3=%d ****************\n",total_mp3);
      if(total_mp3 == 0){
		  	scan_finish = 1;
				LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_FINISH,ITUNES_OK, MIPC_TIME_ALWAYS,(int)scan_finish);
      }else{
      	LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_FINISH,ITUNES_OK, MIPC_TIME_ALWAYS,(int)clean_IPC);
      	reload = 1;
      }
    }
    else{
		...
		LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_TOTAL,ITUNES_OK,
	  MIPC_TIME_ALWAYS,(int)total_mp3);
...
//        rend_register(servername,"_http._tcp",ws_config.port,iface,txtrecord);
	...
	LIB_Get_IPC_int(MIPC_ITUNES,ITUNES_SERVER_READY,&server_flag_test);
    //printf("2: ITUNES_SERVER_READY = %d\n", server_flag_test);

	// Prevent mserver return something unknow value, create file for cgi checking second time.
	fptr=fopen("/tmp/ff_ready.txt", "w");
	fclose(fptr);
    ...
    while(!config.stop) {
        ...
        if(config.reload) {
            ...
            /* scanning is finish */
            scan_finish=1;
						LIB_Set_IPC_int(MIPC_ITUNES,ITUNES_FINISH,ITUNES_OK,
	  				MIPC_TIME_ALWAYS,(int)scan_finish);
        }
